...
namespace osu.Game.Skinning
{
    public abstract class Skin : IDisposable, ISkin
    {
        ...
        /// <summary>
        /// Mosu-specific per-component custom font overrides, stored separately from layout JSON for vanilla compatibility.
        /// </summary>
        ...
        protected Skin(SkinInfo skin, IStorageResourceProvider? resources, IResourceStore<byte[]>? fallbackStore = null, string configurationFilename = @"skin.ini")
        {
            ...
            byte[]? customFontBytes = store?.Get(SkinCustomFontInfo.FILENAME);

            if (customFontBytes != null)
            {
                try
                {
                    string jsonContent = Encoding.UTF8.GetString(customFontBytes);
                    CustomFontInfo = JsonConvert.DeserializeObject<SkinCustomFontInfo>(jsonContent) ?? new SkinCustomFontInfo();
                    CustomFontInfo.Version = SkinCustomFontInfo.LATEST_VERSION;
                }
                catch (Exception ex)
                {
        ...
        public void UpdateDrawableTarget(SkinnableContainer targetContainer)
        {
            ...
            CustomFontInfo.SetOverridesForTarget(
                targetContainer.Lookup.Lookup,
                targetContainer.Lookup.Ruleset,
                SkinCustomFontSerializer.ExtractOverrides(
                    targetContainer.Lookup.Lookup,
                    targetContainer.Lookup.Ruleset,
                    targetContainer.Components.Cast<Drawable>()));
        }
                ...
                case UserSkinComponentLookup userLookup:
                    switch (userLookup.Component)
                    {
                        case GlobalSkinnableContainerLookup containerLookup:
                            ...
                            var componentsContainer = new Container
                            {
                            ...
                            SkinCustomFontSerializer.ApplyOverrides(
                                ...
                                drawableInfos,
                                componentsContainer.Children.ToArray());
        ...
        private SkinLayoutInfo? parseLayoutInfo(string jsonContent, GlobalSkinnableContainers target)
        {
            ...
            {
                Logger.Log($"Deserialising skin layout to {nameof(SkinLayoutInfo)} failed.\nDetails: {ex}");
            }

            // If deserialisation using SkinLayoutInfo fails, attempt to deserialise using the old naked list,
            // but only if the layout actually uses the legacy top-level array format.
            if (layout == null && jsonContent.TrimStart().StartsWith('['))
            {
