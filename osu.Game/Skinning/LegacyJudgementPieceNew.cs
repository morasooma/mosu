...
namespace osu.Game.Skinning
{
    public partial class LegacyJudgementPieceNew : CompositeDrawable, IAnimatableJudgement
    {
        ...
        public LegacyJudgementPieceNew(HitResult result, Func<Drawable> createMainDrawable, Texture? particleTexture)
        {
            ...
            if (particleTexture != null && !SkinPerformanceMode.ShouldSimplifyEffects)
            {
            ...
            if (!result.IsMiss() && !SkinPerformanceMode.ShouldSimplifyEffects)
            {
        ...
        public void PlayAnimation()
        {
            ...
            if (SkinPerformanceMode.ShouldSimplifyEffects)
            {
                animation?.GotoAndStop(0);
                temporaryOldStyle?.Hide();
                particles?.Hide();
                SkinPerformanceMode.ApplyFastJudgementFade(this);
                return;
            }

            animation?.GotoFrame(0);

            if (particles != null && !SkinPerformanceMode.ShouldSimplifyEffects)
            {
