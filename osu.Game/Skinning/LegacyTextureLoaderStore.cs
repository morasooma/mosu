...
namespace osu.Game.Skinning
{
    public class LegacyTextureLoaderStore : IResourceStore<TextureUpload>
    {
        ...
        public TextureUpload Get(string name)
        {
            ...
            textureUpload = shouldConvertToGrayscale(name)
                ? convertToGrayscale(textureUpload)
                : textureUpload;

            return downscaleIfNeeded(name, textureUpload);
        }
        ...
        public Task<TextureUpload> GetAsync(string name, CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            return Task.Run(() =>
            {
                if (shouldConvertToGrayscale(name))
                    textureUpload = convertToGrayscale(textureUpload);
        ...
        private TextureUpload downscaleIfNeeded(string name, TextureUpload textureUpload)
        {
            if (!SkinPerformanceMode.ShouldDownscaleLegacyTexture(name))
                return textureUpload;

            int width = Math.Max(1, (int)MathF.Round(textureUpload.Width * SkinPerformanceMode.LegacyTextureResolutionScale));
            int height = Math.Max(1, (int)MathF.Round(textureUpload.Height * SkinPerformanceMode.LegacyTextureResolutionScale));

            if (width == textureUpload.Width && height == textureUpload.Height)
                return textureUpload;
