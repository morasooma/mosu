    ...
    public static partial class LegacySkinExtensions
    {
                                             ...
                                             string animationSeparator = "-", bool startAtCurrentTime = true, double? frameLength = null, Vector2? maxSize = null)
        {
            ...
            switch (textures.Length)
            {
                ...
                default:
                    if (SkinPerformanceMode.ShouldFreezeAnimations)
                        return new Sprite { Texture = textures[0] };
                                            ...
                                            out ISkin? retrievalSource)
        {
            ...
            if (animatable)
            {
                if (SkinPerformanceMode.ShouldFreezeAnimations)
                {
                    var firstFrame = retrievalSource.GetTexture(getFrameName(0), wrapModeS, wrapModeT);

                    if (firstFrame != null)
                    {
                        if (maxSize != null)
                            firstFrame = firstFrame.WithMaximumSize(maxSize.Value);
        ...
        public partial class SkinnableTextureAnimation : TextureAnimation
        {
            ...
            private IBindable<bool>? skinPerformanceMode;
