    ...
    public class LegacyScoreEncoder
    {
        ...
        public void Encode(Stream stream, bool leaveOpen = false)
        {
            using (SerializationWriter sw = new SerializationWriter(stream, leaveOpen))
            {
                ...
                sw.WriteByteArray(EncodeReplayData());
        ...
        /// <summary>
        /// Encodes only the compressed legacy replay frame payload.
        /// This is the form expected by the stable score submission endpoint, without an .osr header.
        /// </summary>
        public byte[] EncodeReplayData() => LegacyScoreDataCompression.Compress(replayStringContent);

        private byte[] createScoreInfoData() => LegacyScoreDataCompression.Compress(LegacyReplaySoloScoreInfo.FromScore(score.ScoreInfo).Serialize());
