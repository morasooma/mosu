...
namespace osu.Game.Scoring
{
    public class ScoreManager : ModelManager<ScoreInfo>, IModelImporter<ScoreInfo>
    {
            ...
            : base(storage, realm)
        {
            ...
            scoreExporter = new LegacyScoreExporter(storage, configManager)
            {
                PostNotification = obj => PostNotification?.Invoke(obj)
            };
        }
        ...
        /// <summary>
        /// Perform a lookup query on available <see cref="ScoreInfo"/>s, ordered newest first.
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="limit">The maximum number of detached results to return.</param>
        /// <returns>Detached score results matching the provided query.</returns>
        public List<ScoreInfo> QueryLatest(Expression<Func<ScoreInfo, bool>> query, int limit = 50)
        {
        ...
        private ScoreInfo? getDatabasedScoreInfo(IScoreInfo originalScoreInfo)
        {
            ...
            if (originalScoreInfo.OnlineID > 0)
                databasedScoreInfo ??= queryMatchingContext(s => s.OnlineID == originalScoreInfo.OnlineID, originalScoreInfo);

            if (originalScoreInfo.LegacyOnlineID > 0)
                databasedScoreInfo ??= queryMatchingContext(s => s.LegacyOnlineID == originalScoreInfo.LegacyOnlineID, originalScoreInfo);
        ...
        private ScoreInfo? queryMatchingContext(Expression<Func<ScoreInfo, bool>> query, IScoreInfo originalScoreInfo)
        {
            return Realm.Run(r => r.All<ScoreInfo>()
                                   .Where(query)
                                   .AsEnumerable()
                                   .FirstOrDefault(s => s.MatchesScoreContext(originalScoreInfo))
                                   ?.Detach());
        }
        ...
        public override bool IsAvailableLocally(ScoreInfo model)
            => Realm.Run(realm => realm.All<ScoreInfo>()
                                       ...
                                       .Where(s => s.OnlineID == model.OnlineID || s.LegacyOnlineID == model.LegacyOnlineID)
                                       .AsEnumerable()
                                       .Any(s => s.MatchesScoreContext(model)));
        ...
        public Task Export(ScoreInfo scoreInfo)
        {
            ...
            var score = GetScore(databasedScoreInfo);
            if (score == null || databasedScoreInfo.BeatmapInfo == null)
                return Task.CompletedTask;

            var workingBeatmap = beatmaps().GetWorkingBeatmap(databasedScoreInfo.BeatmapInfo.Detach());
            return scoreExporter.ExportAsync(score, workingBeatmap.Beatmap);
        }
