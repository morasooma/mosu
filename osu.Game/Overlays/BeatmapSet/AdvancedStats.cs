...
namespace osu.Game.Overlays.BeatmapSet
{
    public partial class AdvancedStats : Container
    {
        ...
        public partial class StatisticRow : Container, IHasAccentColour, IHasCustomTooltip<RulesetBeatmapAttribute>
        {
            ...
            public (float baseValue, float? adjustedValue) Value
            {
                ...
                set
                {
                    ...
                    if (Precision.AlmostEquals(value.baseValue, value.adjustedValue ?? value.baseValue, 0.05f))
                        ModBar.AccentColour = valueText.Colour = getContentColour();
            ...
            private Color4 getContentColour() => OverlayColourProvider.IsLightTheme ? (Color4)colourProvider.Content1 : Color4.White;

            private void updateThemeColours()
            {
                name.Colour = valueText.Colour = getContentColour();
                bar.BackgroundColour = getContentColour().Opacity(0.5f);

                if (value.HasValue && Precision.AlmostEquals(value.Value.baseValue, value.Value.adjustedValue ?? value.Value.baseValue, 0.05f))
                    ModBar.AccentColour = valueText.Colour = getContentColour();
            }
