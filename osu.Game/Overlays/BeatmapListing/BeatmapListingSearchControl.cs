...
namespace osu.Game.Overlays.BeatmapListing
{
    ...
    {
        private const int dodge_ruleset_id = 10;
        ...
        public IBindable<bool> ServerExclusive => serverExclusiveButton.Active;
        ...
        private readonly ServerExclusiveTabItem serverExclusiveButton;

        private readonly Box background;
        private readonly UpdateableOnlineBeatmapSetCover beatmapCover;
        private IBindable<Colour4> themeColour = null!;
        private bool? serverExclusiveBeforeDodge;
                    ...
                    Child = new FillFlowContainer
                    {
                        ...
                        Children = new Drawable[]
                        {
                            ...
                            new FillFlowContainer
                            {
                                ...
                                Children = new Drawable[]
                                {
                                    serverExclusiveButton = new ServerExclusiveTabItem(),
                                }
                            },
        ...
        {
            themeColour = colourProvider.GetColourBindable(OverlayColour.Dark6);
        ...
        protected override void LoadComplete()
        {
            ...
            serverExclusiveButton.Active.BindValueChanged(active =>
            {
                if (active.NewValue)
                {
                    if (!Extra.Contains(SearchExtra.ServerExclusive))
                        Extra.Add(SearchExtra.ServerExclusive);
                }
                else
                    Extra.Remove(SearchExtra.ServerExclusive);
            }, true);

            Extra.BindCollectionChanged((_, _) =>
            {
                bool contains = Extra.Contains(SearchExtra.ServerExclusive);

                if (!serverExclusiveButton.Active.Disabled)
                    serverExclusiveButton.Active.Value = contains;
            }, true);

            Ruleset.BindValueChanged(ruleset =>
            {
                bool dodgeSelected = ruleset.NewValue?.OnlineID == dodge_ruleset_id;

                if (dodgeSelected)
                {
                    serverExclusiveBeforeDodge ??= serverExclusiveButton.Active.Value;
                    serverExclusiveButton.Active.Value = true;
                    serverExclusiveButton.Active.Disabled = true;
                    return;
                }

                if (serverExclusiveBeforeDodge is not { } previousValue)
                    return;

                serverExclusiveButton.Active.Disabled = false;
                serverExclusiveButton.Active.Value = previousValue;
                serverExclusiveBeforeDodge = null;
            }, true);
        }
        ...
        private partial class ServerExclusiveTabItem : FilterTabItem<SearchExtra>
        {
            private Container activeContent = null!;
            private Circle background = null!;

            public ServerExclusiveTabItem()
                : base(SearchExtra.ServerExclusive)
            {
            }
