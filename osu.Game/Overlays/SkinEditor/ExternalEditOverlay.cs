...
namespace osu.Game.Overlays.SkinEditor
{
    public partial class ExternalEditOverlay : OsuFocusedOverlayContainer
    {
        ...
        public async Task<Task> Begin(SkinInfo skinInfo, bool openSkinIni = false)
        {
            if (taskCompletionSource != null)
                throw new InvalidOperationException("Cannot start multiple concurrent external edits!");

            this.openSkinIni = openSkinIni;
            ...
            Schedule(() =>
            {
                flow.Children = new Drawable[]
                {
                    ...
                    {
                        Text = openSkinIni ? SkinSettingsStrings.OpenSkinIni : EditorStrings.OpenFolder,
                        Width = 350,
                        Anchor = Anchor.TopCentre,
                        Origin = Anchor.TopCentre,
                        Action = openLocation,
        ...
        private void openLocation()
        {
            ...
            string mountedPath = editOperation.MountedPath.TrimDirectorySeparator() + Path.DirectorySeparatorChar;

            if (openSkinIni)
            {
                string skinIniPath = Path.Combine(mountedPath, "skin.ini");

                if (File.Exists(skinIniPath))
                {
                    gameHost.OpenFileExternally(skinIniPath);
                    return;
                }
            }

            gameHost.OpenFileExternally(mountedPath);
        }
