...
{
    [Cached(typeof(ModSelectOverlay))]
    public partial class ModSelectOverlay : ShearedOverlayContainer, ISamplePlaybackDisabler, IKeyBindingHandler<PlatformAction>
    {
        ...
        private OsuConfigManager configManager = null!;
        ...
        private void updateModSettingChangeTracker()
        {
            modSettingChangeTracker?.Dispose();
            modSettingChangeTracker = null;

            if (!AllowCustomisation || SelectedMods.Value.Count == 0)
                return;

            modSettingChangeTracker = new ModSettingChangeTracker(SelectedMods.Value);
            modSettingChangeTracker.SettingChanged += _ => Scheduler.AddOnce(notifySelectedModSettingsChanged);
        }

        private void updateActiveMods() => ActiveMods.Value = ComputeActiveMods().ToArray();

        private void notifySelectedModSettingsChanged()
        {
            // Mod settings mutate the selected mod instances in-place. Re-publish the selection so bind targets and
            // consumers which react to SelectedMods changes see the updated settings.
            if (!ModUtils.CheckCompatibleSet(SelectedMods.Value, out var invalidMods))
            {
                SelectedMods.Value = SelectedMods.Value.Except(invalidMods).ToArray();
            }
            else
            {
                SelectedMods.Value = SelectedMods.Value.ToArray();
            }
        }
        ...
        /// </summary>
        public void DeselectAll() => DeselectAll(false);

        private void DeselectAll(bool preserveClassic)
        {
            foreach (var column in columnFlow.Columns.OfType<ModColumn>())
                column.DeselectAll(preserveClassic);
        }
        ...
        private IEnumerable<ColumnDimContainer> createColumns()
        {
            ...
            if (!Online.MosuServerEnvironment.IsThirdPartyServer)
                yield return createModColumnContent(ModType.Mosu);
        ...
        private void updateFromExternalSelection()
        {
            ...
            foreach (var modState in AllAvailableMods)
            {
                ...
                {
                    if (!ReferenceEquals(modState.Mod, matchingSelectedMod))
        ...
        public override bool OnPressed(KeyBindingPressEvent<GlobalAction> e)
        {
            ...
            switch (e.Action)
            {
                ...
                case GlobalAction.DeselectAllMods:
                {
                    ...
                    {
                        if (configManager.Get<bool>(OsuSetting.ForkClassicModDefault))
                            DeselectAll(preserveClassic: true);
                        else
