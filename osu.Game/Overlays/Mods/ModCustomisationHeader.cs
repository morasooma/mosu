...
namespace osu.Game.Overlays.Mods
{
    public partial class ModCustomisationHeader : OsuClickableContainer
    {
        ...
        protected override void LoadComplete()
        {
            ...
            ExpandedState.BindValueChanged(_ => updateExpandedState(), true);

            themeColour = colourProvider.GetColourBindable(OverlayColour.Dark3);
            themeColour.BindValueChanged(_ => updateExpandedState(), true);
        }

        private void updateExpandedState()
        {
            icon.ScaleTo(ExpandedState.Value > ModCustomisationPanelState.Collapsed ? new Vector2(1, -1) : Vector2.One,
                OverlayColourProvider.ThemeTransitionDuration(300), Easing.OutQuint);

            switch (ExpandedState.Value)
            {
                case ModCustomisationPanelState.Collapsed:
                    background.FadeColour(colourProvider.Dark3, OverlayColourProvider.ThemeTransitionDuration(500), Easing.OutQuint);
                    break;

                case ModCustomisationPanelState.Expanded:
                case ModCustomisationPanelState.ExpandedByMod:
                    background.FadeColour(colourProvider.Light4, OverlayColourProvider.ThemeTransitionDuration(500), Easing.OutQuint);
                    break;
            }
        }
