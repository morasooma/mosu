...
namespace osu.Game.Overlays.Toolbar
{
    public partial class ToolbarRulesetSelector : RulesetSelector
    {
        ...
        private readonly Dictionary<string, Sample> rulesetSelectionSample = new Dictionary<string, Sample>();
        private readonly Dictionary<string, SampleChannel> rulesetSelectionChannel = new Dictionary<string, SampleChannel>();
        ...
        private void load(AudioManager audio, OsuGameBase game)
        {
            ...
            foreach (var r in Rulesets.AvailableRulesets)
            {
                ...
                rulesetSelectionSample[r.ShortName] = samples.Get($@"UI/ruleset-select-{r.ShortName}");
            }
        ...
        private void playRulesetSelectionSample(ValueChangedEvent<RulesetInfo> r)
        {
            ...
            string sampleKey = r.NewValue.ShortName;

            if (!rulesetSelectionSample.TryGetValue(sampleKey, out var sample) && r.NewValue.IsSpecialRuleset())
                rulesetSelectionSample.TryGetValue(r.NewValue.CreateNormalRuleset().ShortName, out sample);
            ...
            rulesetSelectionChannel[sampleKey] = channel;
            channel.Play();
