    ...
    public partial class MusicController : CompositeDrawable
    {
        ...
        protected override void LoadComplete()
        {
            ...
            mods.BindValueChanged(_ =>
            {
                ...
                modSettingChangeTracker.SettingChanged += _ => Scheduler.AddOnce(ResetTrackAdjustments);
        ...
        private AudioEffectsComponent? modAudioEffectsComponent;
        ...
        {
            if (modAudioEffectsComponent != null)
            {
                RemoveInternal(modAudioEffectsComponent, true);
                modAudioEffectsComponent = null;
            }
            ...
            {
                double rawRate = ModUtils.CalculateRateWithMods(mods.Value);

                if (!double.IsFinite(rawRate) || rawRate <= 0)
                    return;

                var currentModTrackAdjustments = modTrackAdjustments = new AudioAdjustments();
                CurrentTrack.BindAdjustments(currentModTrackAdjustments);

                var safeTrackAdjustments = new SafeRateAdjustableAudioComponent(currentModTrackAdjustments);

                foreach (var mod in mods.Value.OfType<IApplicableToTrack>())
                    mod.ApplyToTrack(safeTrackAdjustments);

                // Mixer effects cannot be represented by IApplicableToTrack adjustments.
                // Attach them here so mod presets can be heard while selecting a beatmap.
                ModAudioEffects? audioEffects = mods.Value.OfType<ModAudioEffects>().SingleOrDefault();

                if (audioEffects != null)
                    AddInternal(modAudioEffectsComponent = audioEffects.CreateEffectsComponent(false));
            }
        }
