...
namespace osu.Game.Configuration
{
    ...
    /// Can be used in conjunction with <see cref="SettingSourceExtensions.CreateSettingsControls(object)"/> to automatically create UI controls.
    /// </summary>
    /// <remarks>
    ...
    public class SettingSourceAttribute : Attribute, IComparable<SettingSourceAttribute>
    {
        ...
        /// <summary>
        /// Whether this setting should be excluded from drawable serialisation.
        /// Useful for UI-only settings controls that mirror other serialised state.
        /// </summary>
        public bool ExcludeFromSerialisation { get; set; }

        /// <summary>
        /// Whether this setting should be excluded from auto-generated settings controls.
        /// Useful for serialised state that is edited via a separate UI-only setting.
        /// </summary>
        public bool ExcludeFromSettingsControls { get; set; }
    ...
    public static partial class SettingSourceExtensions
    {
        public static IEnumerable<Drawable> CreateSettingsControls(this object obj)
            => CreateSettingsControls(obj, false);

        public static IEnumerable<Drawable> CreateSettingsControls(this object obj, bool allowNumericInput)
        {
            foreach (var (attr, property) in obj.GetOrderedSettingsSourceProperties())
            {
                if (attr.ExcludeFromSettingsControls)
                    continue;

                object value = property.GetValue(obj)!;
                yield return createSettingControl(obj, attr, value);

                Drawable? exactInput = createExactNumericInput(value, allowNumericInput);

                if (exactInput != null)
                    yield return exactInput;
            }
        }

        private static Drawable createSettingControl(object obj, SettingSourceAttribute attr, object value)
        {
            ...
            return value switch
            {
                ...
                BindableColour4 bColour => new SettingsColour
                {
                    LabelText = attr.Label,
                    TooltipText = attr.Description,
                    Current = bColour
                },
                IBindable bindable => createDropdown(attr, bindable),
                _ => throw new InvalidOperationException($"{nameof(SettingSourceAttribute)} was attached to an unsupported type ({value})")
            };
        }

        private static Drawable createDropdown(SettingSourceAttribute attr, IBindable bindable)
        {
        ...
        private static Drawable? createExactNumericInput(object value, bool allowNumericInput)
        {
            ...
            return value switch
            {
                DifficultyBindable difficultyBindable => new SettingsDifficultyTextBox
                {
                    Current = difficultyBindable,
                },
                BindableNumber<float> bNumber => new SettingsNumericTextBox<float>
                {
                    Current = bNumber,
                },
                BindableNumber<double> bNumber => new SettingsNumericTextBox<double>
                {
                    Current = bNumber,
                },
                BindableNumber<int> bNumber => new SettingsNumericTextBox<int>
                {
                    Current = bNumber,
                },
                _ => null
        ...
        public static IEnumerable<(SettingSourceAttribute, PropertyInfo)> GetSettingsSourceProperties(this object obj)
        {
            ...
            {
                return properties.Where(p => !(obj is ModRateAdjust && p.Item2.Name == nameof(ModRateAdjust.LockDifficultyAdjust)));
            }
