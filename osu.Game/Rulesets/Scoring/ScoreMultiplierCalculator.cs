...
namespace osu.Game.Rulesets.Scoring
{
    ...
    public class ScoreMultiplierCalculator
    {
        ...
        /// </summary>
        /// <remarks>
        /// In Ranked Play mode all mod multipliers are forced to 1.00x to keep the playing field fair.
        /// </remarks>
        public double CalculateFor(IEnumerable<Mod> mods)
        {
            // In Ranked Play all mods have a neutral 1.00x multiplier.
            if (Context.IsRankedPlay)
                return 1.0;

            // Some legacy scores may contain the same mod more than once (including scores
            // produced by old exploits). Treat duplicate mod types as a single mod rather
            // than allowing ToDictionary() to throw while displaying such a score.
            var allModsByType = mods
    ...
    public class ScoreMultiplierContext
    {
        ...
        /// <summary>
        /// When <see langword="true"/>, indicates the score is being played in Ranked Play mode.
        /// In this mode all mod score multipliers are forced to 1.00x to ensure a level playing field.
        /// </summary>
        public bool IsRankedPlay { get; }
        ...
        /// </param>
        /// <param name="isRankedPlay">
        /// Pass <see langword="true"/> when the score is being played in Ranked Play mode.
        /// All mod multipliers will be forced to 1.00x.
        /// </param>
        public ScoreMultiplierContext(IBeatmapDifficultyInfo beatmapDifficultyWithoutMods, ScoreInfo? score = null, bool isRankedPlay = false)
        {
