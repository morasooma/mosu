...
namespace osu.Game.Rulesets
{
    public abstract class RulesetStore : IDisposable, IRulesetStore
    {
        ...
        public RulesetInfo? GetRuleset(int id)
            => AvailableRulesets.FirstOrDefault(r => r.OnlineID == id) ?? createSpecialRuleset(id);
        ...
        public RulesetInfo? GetRuleset(string shortName)
            => AvailableRulesets.FirstOrDefault(r => r.ShortName == shortName) ?? createSpecialRuleset(shortName);

        private RulesetInfo? createSpecialRuleset(int id)
        {
            return id switch
            {
                RulesetInfo.OSU_RELAX_ONLINE_ID => AvailableRulesets.FirstOrDefault(r => r.OnlineID == 0)?.CreateSpecialRuleset(RulesetInfo.OSU_RELAX_MODE_SHORTNAME, id),
                RulesetInfo.OSU_AUTOPILOT_ONLINE_ID => AvailableRulesets.FirstOrDefault(r => r.OnlineID == 0)?.CreateSpecialRuleset(RulesetInfo.OSU_AUTOPILOT_MODE_SHORTNAME, id),
                RulesetInfo.TAIKO_RELAX_ONLINE_ID => AvailableRulesets.FirstOrDefault(r => r.OnlineID == 1)?.CreateSpecialRuleset(RulesetInfo.TAIKO_RELAX_MODE_SHORTNAME, id),
                RulesetInfo.CATCH_RELAX_ONLINE_ID => AvailableRulesets.FirstOrDefault(r => r.OnlineID == 2)?.CreateSpecialRuleset(RulesetInfo.CATCH_RELAX_MODE_SHORTNAME, id),
                _ => null
            };
        }

        private RulesetInfo? createSpecialRuleset(string shortName)
        {
            return shortName switch
            {
                RulesetInfo.OSU_RELAX_MODE_SHORTNAME => AvailableRulesets.FirstOrDefault(r => r.ShortName == RulesetInfo.OSU_MODE_SHORTNAME)?.CreateSpecialRuleset(shortName, RulesetInfo.OSU_RELAX_ONLINE_ID),
                RulesetInfo.OSU_AUTOPILOT_MODE_SHORTNAME => AvailableRulesets.FirstOrDefault(r => r.ShortName == RulesetInfo.OSU_MODE_SHORTNAME)?.CreateSpecialRuleset(shortName, RulesetInfo.OSU_AUTOPILOT_ONLINE_ID),
                RulesetInfo.TAIKO_RELAX_MODE_SHORTNAME => AvailableRulesets.FirstOrDefault(r => r.ShortName == RulesetInfo.TAIKO_MODE_SHORTNAME)?.CreateSpecialRuleset(shortName, RulesetInfo.TAIKO_RELAX_ONLINE_ID),
                RulesetInfo.CATCH_RELAX_MODE_SHORTNAME => AvailableRulesets.FirstOrDefault(r => r.ShortName == RulesetInfo.CATCH_MODE_SHORTNAME)?.CreateSpecialRuleset(shortName, RulesetInfo.CATCH_RELAX_ONLINE_ID),
                _ => null
            };
        }
