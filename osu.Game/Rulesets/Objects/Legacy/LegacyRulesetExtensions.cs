...
namespace osu.Game.Rulesets.Objects.Legacy
{
    public static class LegacyRulesetExtensions
    {
        ...
        public static float CalculateScaleFromCircleSize(float circleSize, bool applyFudge = false) =>
            SanitizeScale(CalculateSignedScaleFromCircleSize(circleSize, applyFudge));

        /// <summary>
        /// Calculates the raw signed scale from a CS value, with an optional fudge that was historically applied to the osu! ruleset.
        /// </summary>
        public static float CalculateSignedScaleFromCircleSize(float circleSize, bool applyFudge = false)
        {
        ...
        /// <summary>
        /// Ensures size-like scales stay positive even when out-of-range values would otherwise make them zero or negative.
        /// </summary>
        public static float SanitizeScale(float scale) => MathF.Max(0.01f, MathF.Abs(scale));

        /// <summary>
        /// Ensures signed scales stay away from zero while preserving their sign, allowing broken logic without zero/infinity blowups.
        /// </summary>
        public static float SanitizeSignedScale(float scale)
        {
            if (MathF.Abs(scale) >= 0.01f)
                return scale;

            return MathF.CopySign(0.01f, scale == 0 ? 1 : scale);
        }
