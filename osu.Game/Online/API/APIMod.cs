...
namespace osu.Game.Online.API
{
    ...
    public class APIMod : IEquatable<APIMod>
    {
        ...
        public APIMod(Mod mod)
        {
            ...
            if (mod is IProvideAPIModSettings apiModSettingsProvider)
                apiModSettingsProvider.AddAPIModSettings(Settings);
        }
        ...
        public Mod ToMod(Ruleset ruleset)
        {
            ...
            if (Settings.Count > 0)
            {
                ...
                foreach (var (_, property) in resultMod.GetSettingsSourceProperties())
                {
                    ...
                    if (!Settings.TryGetValue(settingKey, out object? settingValue))
                        continue;
                ...
                if (resultMod is IProvideAPIModSettings apiModSettingsProvider)
                {
                    foreach (var (settingKey, settingValue) in Settings)
                    {
                        if (appliedSettings.Contains(settingKey))
                            continue;

                        try
                        {
                            apiModSettingsProvider.TryApplyAPIModSetting(settingKey, settingValue);
                        }
                        catch (Exception ex)
                        {
                            Logger.Log($"Failed to apply extra API mod setting '{settingKey}' with value '{settingValue}': {ex.Message}");
                        }
                    }
                }
            }
