...
namespace osu.Game.Online.Chat
{
    ...
    public partial class MessageNotifier : Component
    {
        ...
        private void checkNewMessages(IEnumerable<Message> messages)
        {
            ...
            // A reconnect may transiently retain an older channel instance while the refreshed
            // instance is being joined. Notifications are best-effort, so select the active
            // matching channel instead of throwing when both instances carry the same ID.
            long channelId = messages.First().ChannelId;
            var matchingChannels = channelManager.JoinedChannels.Where(c => c.Id > 0 && c.Id == channelId);
            var channel = matchingChannels.FirstOrDefault(c => c == channelManager.CurrentChannel.Value)
                          ?? matchingChannels.FirstOrDefault();
