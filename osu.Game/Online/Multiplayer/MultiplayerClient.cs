...
namespace osu.Game.Online.Multiplayer
{
        ...
        IRankedPlayClient, IRankedPlayServer
    {
        ...
        {
            MultiplayerRoomUser[] missingUsers = multiplayerUsers.Where(u => u.User == null).ToArray();

            foreach (int[] userChunk in missingUsers.Select(u => u.UserID).Distinct().Chunk(GetUsersRequest.MAX_IDS_PER_REQUEST))
            {
                ...
                foreach (var multiplayerUser in missingUsers)
                {
        ...
        public abstract Task Surrender();
        ...
        public abstract Task MatchmakingJoinQueue(int poolId, bool randomMods = false);

        Task IMatchmakingServer.MatchmakingJoinQueue(int poolId) => MatchmakingJoinQueue(poolId);

        Task IMatchmakingServer.MatchmakingJoinQueueWithParams(MatchmakingJoinQueueRequest request)
            => MatchmakingJoinQueue(request.PoolId, request.RandomMods);
