...
namespace osu.Game.Online
{
    ...
    public partial class OnlineStatusNotifier : Component
    {
        ...
        private readonly HashSet<string> seenSystemNotificationIds = new HashSet<string>();
        private readonly Queue<string> seenSystemNotificationOrder = new Queue<string>();
        private const int max_cached_system_notifications = 32;
        ...
        private void load(IAPIProvider api)
        {
            ...
            multiplayerClient.SystemNotificationReceived += showSystemNotification;
            spectatorClient.SystemNotificationReceived += showSystemNotification;
            metadataClient.SystemNotificationReceived += showSystemNotification;
        }
        ...
        private void showSystemNotification(string notificationId, string message)
        {
            ...
            {
                if (!seenSystemNotificationIds.Add(notificationId))
                    return;

                seenSystemNotificationOrder.Enqueue(notificationId);

                while (seenSystemNotificationOrder.Count > max_cached_system_notifications)
                {
                    string expiredId = seenSystemNotificationOrder.Dequeue();
                    seenSystemNotificationIds.Remove(expiredId);
                }
            }
