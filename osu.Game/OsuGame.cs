...
namespace osu.Game
{
    ...
    public partial class OsuGame : OsuGameBase, IKeyBindingHandler<GlobalAction>, ILocalUserPlayInfo, IPerformFromScreenRunner, IOverlayManager, ILinkHandler
    {
        ...
        /// </summary>
        public static readonly Bindable<bool> DisableShear = new Bindable<bool>();
        public static Vector2 SHEAR => DisableShear.Value ? Vector2.Zero : new Vector2(0.2f, 0);
        ...
        private const int framework_unlimited_frame_limit = 1000;
        ...
        private SupporterQuestsOverlay supporterQuestsOverlay;
        private int lastCompletedQuestsCount = -1;
        ...
        private Bindable<bool> uncappedFrameRate;
        private Bindable<bool> limitMenuFps2x;
        private Bindable<bool> disableShear;
        private bool? benchmarkUnlimitedFramesState;
        private bool isMenuFpsLimited;
        ...
        private Bindable<bool> separateSkinsPerRuleset;
        private Dictionary<int, Bindable<string>> rulesetSkins;
        ...
        public override void SetHost(GameHost host)
        {
            ...
            // SkinManager and the built-in skins are constructed during dependency loading, before
            // this game's BackgroundDependencyLoader runs. Seed the static performance state from
            // persisted configuration now so their constructor/load-time branches never observe the
            // hard-coded defaults. A process-level diagnostics override, when present, remains the
            // effective value while these assignments update only its backing configuration.
            SkinPerformanceMode.Enabled = LocalConfig.Get<bool>(OsuSetting.ForkSkinPerformanceMode);
            SkinPerformanceMode.FreezeAnimations = LocalConfig.Get<bool>(OsuSetting.ForkSkinPerformanceFreezeAnimations);
            SkinPerformanceMode.SimplifyEffects = LocalConfig.Get<bool>(OsuSetting.ForkSkinPerformanceSimplifyEffects);
            SkinPerformanceMode.OptimiseTextures = LocalConfig.Get<bool>(OsuSetting.ForkSkinPerformanceOptimiseTextures);
            SkinPerformanceMode.SimplifyHud = LocalConfig.Get<bool>(OsuSetting.ForkSkinPerformanceSimplifyHud);
            SkinPerformanceMode.SimplifyCounters = LocalConfig.Get<bool>(OsuSetting.ForkSkinPerformanceSimplifyCounters);
            SkinPerformanceMode.DisableKiaiFlashing = LocalConfig.Get<bool>(OsuSetting.ForkSkinPerformanceDisableKiaiFlashing);
            SkinPerformanceMode.BlackBackground = LocalConfig.Get<bool>(OsuSetting.ForkSkinPerformanceBlackBackground);
            SkinPerformanceMode.ArgonFollowRing = LocalConfig.Get<bool>(OsuSetting.ForkArgonFollowRing);
        ...
        private void load()
        {
            ...
            uncappedFrameRate = LocalConfig.GetBindable<bool>(OsuSetting.ForkUncappedFrameRate);
            limitMenuFps2x = LocalConfig.GetBindable<bool>(OsuSetting.ForkLimitMenuFps2x);
            frameSyncMode.BindValueChanged(_ => updateBenchmarkUnlimitedFrameState(), true);
            ...
            void reloadSkinPerformanceSettings()
            {
                ...
                SkinPerformanceMode.NotifySettingsChanged();
            }

            void bindSkinPerformanceSetting(OsuSetting setting, Action<bool> apply)
            {
                LocalConfig.GetBindable<bool>(setting).BindValueChanged(v =>
                {
                    apply(v.NewValue);

                    // Rebuild already-created skinnable components so that the change also
                    // takes effect in the active play session.
                    if (skinPerformanceSettingsInitialised)
                        Scheduler.AddOnce(reloadSkinPerformanceSettings);
                }, true);
            }

            bindSkinPerformanceSetting(OsuSetting.ForkSkinPerformanceMode, value => SkinPerformanceMode.Enabled = value);
            bindSkinPerformanceSetting(OsuSetting.ForkSkinPerformanceFreezeAnimations, value => SkinPerformanceMode.FreezeAnimations = value);
            bindSkinPerformanceSetting(OsuSetting.ForkSkinPerformanceSimplifyEffects, value => SkinPerformanceMode.SimplifyEffects = value);
            bindSkinPerformanceSetting(OsuSetting.ForkSkinPerformanceOptimiseTextures, value => SkinPerformanceMode.OptimiseTextures = value);
            bindSkinPerformanceSetting(OsuSetting.ForkSkinPerformanceSimplifyHud, value => SkinPerformanceMode.SimplifyHud = value);
            bindSkinPerformanceSetting(OsuSetting.ForkSkinPerformanceSimplifyCounters, value => SkinPerformanceMode.SimplifyCounters = value);
            bindSkinPerformanceSetting(OsuSetting.ForkSkinPerformanceDisableKiaiFlashing, value => SkinPerformanceMode.DisableKiaiFlashing = value);
            // Without this startup binding the blackout checkbox only took effect after opening the
            // settings panel (its static was applied solely by SkinPerformanceSettingsPanel).
            ...
            {
                configRuleset.Value = r.NewValue.ShortName;
                applySkinForCurrentRuleset();
            };
            ...
            separateSkinsPerRuleset = LocalConfig.GetBindable<bool>(OsuSetting.ForkSeparateSkinsPerRuleset);
            rulesetSkins = new Dictionary<int, Bindable<string>>
            {
                [0] = LocalConfig.GetBindable<string>(OsuSetting.ForkOsuSkin),
                [1] = LocalConfig.GetBindable<string>(OsuSetting.ForkTaikoSkin),
                [2] = LocalConfig.GetBindable<string>(OsuSetting.ForkCatchSkin),
                [3] = LocalConfig.GetBindable<string>(OsuSetting.ForkManiaSkin),
                [10] = LocalConfig.GetBindable<string>(OsuSetting.ForkDodgeSkin),
            };

            // Transfer any runtime changes back to either the global skin or the active ruleset's skin.
            SkinManager.CurrentSkinInfo.ValueChanged += skin =>
            {
                string skinId = skin.NewValue.ID.ToString();

                if (separateSkinsPerRuleset.Value && tryGetCurrentRulesetSkin(out var rulesetSkin))
                    rulesetSkin.Value = skinId;
                else
                    configSkin.Value = skinId;
            };

            foreach (var rulesetSkin in rulesetSkins.Values)
            {
                rulesetSkin.ValueChanged += _ =>
                {
                    if (separateSkinsPerRuleset.Value
                        && tryGetCurrentRulesetSkin(out var currentRulesetSkin)
                        && ReferenceEquals(rulesetSkin, currentRulesetSkin))
                        applySkinForCurrentRuleset();
                };
            }

            separateSkinsPerRuleset.BindValueChanged(enabled =>
            {
                if (enabled.NewValue)
                {
                    foreach (var rulesetSkin in rulesetSkins.Values)
                    {
                        if (string.IsNullOrWhiteSpace(rulesetSkin.Value))
                            rulesetSkin.Value = configSkin.Value;
                    }
        ...
        private bool tryGetCurrentRulesetSkin(out Bindable<string> rulesetSkin)
        {
            rulesetSkin = null;

            if (rulesetSkins == null || Ruleset.Value == null)
                return false;

            int rulesetId = Ruleset.Value.CreateNormalRuleset().OnlineID;

            return rulesetSkins.TryGetValue(rulesetId, out rulesetSkin);
        }

        private void applySkinForCurrentRuleset()
        {
            if (separateSkinsPerRuleset?.Value != true || !tryGetCurrentRulesetSkin(out var rulesetSkin))
                return;

            string configuredSkin = rulesetSkin.Value;

            if (!Guid.TryParse(configuredSkin, out Guid configuredSkinId) || SkinManager.Query(s => s.ID == configuredSkinId && !s.DeletePending) == null)
                configuredSkin = configSkin.Value;

            SkinManager.SetSkinFromConfiguration(configuredSkin);

            // Normalise empty, invalid or deleted selections to the skin which was actually loaded.
            string loadedSkin = SkinManager.CurrentSkinInfo.Value.ID.ToString();

            if (rulesetSkin.Value != loadedSkin)
                rulesetSkin.Value = loadedSkin;
        }

        protected override void Update()
        {
            base.Update();
            updateBenchmarkUnlimitedFrameState();
        }

        private void updateBenchmarkUnlimitedFrameState()
        {
            if (Host == null || frameSyncMode == null || uncappedFrameRate == null || limitMenuFps2x == null)
                return;

            bool isInGameplayOrReplay = ScreenStack?.CurrentScreen is Player || ScreenStack?.CurrentScreen is PlayerLoader;

            if (limitMenuFps2x.Value && !isInGameplayOrReplay)
            {
                Host.AllowBenchmarkUnlimitedFrames = false;
                int refreshRate = (int)MathF.Round(Host.Window?.CurrentDisplayMode.Value.RefreshRate ?? 60);
                if (refreshRate <= 0) refreshRate = 60;
                double menuDrawLimit = refreshRate * 2;
                double menuUpdateLimit = refreshRate * 4;

                applyFrameRateLimit(menuDrawLimit, menuUpdateLimit);
                isMenuFpsLimited = true;
                benchmarkUnlimitedFramesState = false;
                return;
            }

            if (isMenuFpsLimited)
            {
                isMenuFpsLimited = false;
                frameSyncMode.TriggerChange();
            }

            bool desiredState = uncappedFrameRate.Value && frameSyncMode.Value == FrameSync.Unlimited;

            if (desiredState)
            {
                benchmarkUnlimitedFramesState = true;
                Host.AllowBenchmarkUnlimitedFrames = true;
                applyFrameRateLimit(uncapped_frame_limit, 8000);
                return;
            }

            if (benchmarkUnlimitedFramesState == true)
            {
                Host.AllowBenchmarkUnlimitedFrames = false;
                applyFrameRateLimit(applySaneFrameLimit(Host.MaximumDrawHz), applySaneFrameLimit(Host.MaximumUpdateHz));
            }

            benchmarkUnlimitedFramesState = false;
        }

        private static double applySaneFrameLimit(double currentLimit) => currentLimit <= 0 ? framework_unlimited_frame_limit : Math.Min(framework_unlimited_frame_limit, currentLimit);

        private void applyFrameRateLimit(double drawLimit, double? updateLimit = null)
        {
            updateLimit ??= drawLimit;

            if (Host.MaximumDrawHz != drawLimit)
                Host.MaximumDrawHz = drawLimit;

            if (Host.MaximumUpdateHz != updateLimit.Value)
                Host.MaximumUpdateHz = updateLimit.Value;
        }
        ...
        public void HandleLink(LinkDetails link) => Schedule(() =>
        {
            ...
            switch (link.Action)
            {
                ...
                case LinkAction.DownloadBeatmapSet:
                    if (int.TryParse(argString, out setId))
                        DownloadBeatmapSet(setId);
                    break;
        ...
        /// <summary>
        /// Starts downloading a beatmap set referenced by an osu://dl/{setId} link.
        /// </summary>
        public void DownloadBeatmapSet(int setId)
        {
            var request = new GetBeatmapSetRequest(setId);
            request.Success += beatmapSet => BeatmapDownloader.Download(beatmapSet, LocalConfig.Get<bool>(OsuSetting.PreferNoVideo));
            API.Queue(request);
        }
        ...
        /// <summary>
        /// Show the performance diagnostics setup overlay.
        /// </summary>
        public void ShowPerformanceDiagnosticsSetup() => waitForReady(() => performanceDiagnosticsOverlay, overlay => overlay.ShowSetup());
        ...
        public void PresentScore(IScoreInfo score, ScorePresentType presentType = ScorePresentType.Results)
        {
            ...
            var scoreLeaderboardRuleset = databasedScore.ScoreInfo.Ruleset.CreateSpecialRulesetByScore(databasedScore.ScoreInfo)
                                          ?? databasedScore.ScoreInfo.Ruleset;

            bool selectedRulesetMatchesScore = Ruleset.Value.CreateNormalRuleset().Equals(databasedScore.ScoreInfo.Ruleset.CreateNormalRuleset());
            ...
            IEnumerable<Type> validScreens =
                Beatmap.Value.BeatmapInfo.Equals(databasedBeatmap) && selectedRulesetMatchesScore
            ...
            PerformFromScreen(screen =>
            {
                ...
                if (!Ruleset.Value.CreateNormalRuleset().Equals(databasedScore.ScoreInfo.Ruleset.CreateNormalRuleset()))
                ...
                bool leaderboardRulesetMatches = currentLeaderboard != null && scoreLeaderboardRuleset.Equals(currentLeaderboard.Ruleset);

                if (!leaderboardBeatmapMatches || !leaderboardRulesetMatches)
                {
                    var newLeaderboard = currentLeaderboard != null
                        ? currentLeaderboard with { Beatmap = databasedBeatmap, Ruleset = scoreLeaderboardRuleset }
                        : new LeaderboardCriteria(databasedBeatmap, scoreLeaderboardRuleset, BeatmapLeaderboardScope.Global, null);
        ...
        protected override void LoadComplete()
        {
            ...
            if (OfficialOsuBeatmapApi != null)
                OfficialOsuBeatmapApi.PostNotification = n => Notifications.Post(n);
            ...
            loadComponentSingleFile(supporterQuestsOverlay = new SupporterQuestsOverlay(), overlayContent.Add, true);
            supporterQuestsOverlay.QuestsLoaded += count => lastCompletedQuestsCount = count;
            ...
            loadComponentSingleFile(performanceDiagnosticsOverlay = new PerformanceDiagnosticsOverlay(), topMostOverlayContent.Add, true);
            ...
            // Start supporter quests background polling
            Scheduler.AddDelayed(checkSupporterQuests, 15000, true);
        ...
        private void showServerInfoNotification()
        {
            Scheduler.AddDelayed(() =>
            {
                ...
                Notifications.Post(new ServerInfoNotification(currentServerUrl) { IsImportant = false, Transient = true });
            }, 2000);
        }
        ...
        public bool OnPressed(KeyBindingPressEvent<GlobalAction> e)
        {
            ...
            switch (e.Action)
            {
                ...
                case GlobalAction.RandomSkin:
                    ...
                    displayRulesetSkinChange(GlobalAction.RandomSkin);
                    return true;
                ...
                case GlobalAction.NextSkin:
                    ...
                    displayRulesetSkinChange(GlobalAction.NextSkin);
                    return true;
                ...
                case GlobalAction.PreviousSkin:
                    ...
                    displayRulesetSkinChange(GlobalAction.PreviousSkin);
                    return true;
            }
        ...
        private void displayRulesetSkinChange(GlobalAction action)
        {
            if (separateSkinsPerRuleset?.Value == true)
                onScreenDisplay?.Display(new RulesetSkinChangeToast(SkinManager.CurrentSkinInfo.Value.ToString(), action));
        }

        private partial class RulesetSkinChangeToast : Toast
        {
            private readonly GlobalAction action;

            public RulesetSkinChangeToast(string skinName, GlobalAction action)
                : base(SkinSettingsStrings.SkinSectionHeader, skinName)
            {
                this.action = action;
            }
        ...
        {
            // Kill any active replay render processes before the game exits.
            ReplayRenderProcessRegistry.KillAll();
        ...
        protected virtual void ScreenChanged([CanBeNull] IOsuScreen current, [CanBeNull] IOsuScreen newScreen)
        {
            ...
            // Check supporter quests when screen changes (e.g. gameplay ends)
            checkSupporterQuests();
        }
        ...
        private void checkSupporterQuests()
        {
            ...
            req.Success += quests => Schedule(() =>
            {
                ...
                {
                    lastCompletedQuestsCount = completedCount;
                    return;
                }

                if (completedCount > lastCompletedQuestsCount)
                {
                    var completedList = quests.Where(q => q.Completed).ToList();
                    if (lastCompletedQuestsCount < completedList.Count)
                    {
                        var newlyCompleted = completedList[lastCompletedQuestsCount];
                        Notifications.Post(new SimpleNotification
                        {
                            Text = ForkSettingsStrings.SupporterQuestCompletedNotification(getQuestName(newlyCompleted.Id, newlyCompleted.Name)),
                            Icon = FontAwesome.Solid.CheckCircle,
                            Activated = () =>
                            {
                                Schedule(() => supporterQuestsOverlay.Show());
                                return true;
                            }
                        });
                    }
                ...
                else if (completedCount < lastCompletedQuestsCount)
                {
                    lastCompletedQuestsCount = completedCount;
                }
            });
        ...
        private static LocalisableString getQuestName(int id, string defaultName)
        {
            return ForkSettingsStrings.GetQuestName(id, defaultName);
        }
    }
}
