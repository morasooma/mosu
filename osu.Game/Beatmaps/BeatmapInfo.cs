...
namespace osu.Game.Beatmaps
{
    ...
    public class BeatmapInfo : RealmObject, IHasGuidPrimaryKey, IBeatmapInfo, IEquatable<BeatmapInfo>
    {
        ...
        /// <summary>
        /// The maximum performance points achievable on this beatmap (SS, no mods).
        /// Stored in fork.realm via ForkDataStore, not in the main realm database.
        /// </summary>
        [Ignored]
        public double MaxPerformancePoints { get; set; } = -1;
        ...
        private static bool compareFiles(BeatmapInfo x, BeatmapInfo y, Func<IBeatmapMetadataInfo, string> getFilename)
        {
            ...
            string? fileHashX = x.BeatmapSet.GetFile(fileNameX)?.File.Hash;
            string? fileHashY = y.BeatmapSet.GetFile(fileNameY)?.File.Hash;

            if (fileHashX != null || fileHashY != null)
                return fileHashX == fileHashY;

            if (string.IsNullOrEmpty(fileNameX) && string.IsNullOrEmpty(fileNameY))
                return true;

            return fileNameX == fileNameY && x.BeatmapSet.ID == y.BeatmapSet.ID;
        }
