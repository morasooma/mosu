...
namespace osu.Game.Beatmaps
{
    ...
    public partial class DifficultyRecommender : Component
    {
        ...
        private readonly BindableBool customRecommendedDifficultyEnabled = new BindableBool();
        private readonly BindableDouble customRecommendedDifficulty = new BindableDouble();
        ...
        private void load(Configuration.OsuConfigManager config)
        {
            config.BindWith(Configuration.OsuSetting.ForkCustomRecommendedDifficultyEnabled, customRecommendedDifficultyEnabled);
            config.BindWith(Configuration.OsuSetting.ForkCustomRecommendedDifficulty, customRecommendedDifficulty);
            customRecommendedDifficultyEnabled.BindValueChanged(_ => StarRatingUpdated?.Invoke());
            customRecommendedDifficulty.BindValueChanged(_ => StarRatingUpdated?.Invoke());
        ...
        {
            if (customRecommendedDifficultyEnabled.Value)
                return customRecommendedDifficulty.Value;

            return recommendedDifficultyMapping.TryGetValue(ruleset.ShortName, out double starRating) ? starRating : null;
        }
        ...
        {
            if (customRecommendedDifficultyEnabled.Value)
                return getClosestBeatmap(beatmaps, customRecommendedDifficulty.Value);
            ...
            foreach (string r in orderedRulesets)
            {
                ...
                BeatmapInfo? beatmapInfo = getClosestBeatmap(beatmaps.Where(b => b.Ruleset.ShortName.Equals(r, StringComparison.Ordinal)), recommendation);
        ...
        private static BeatmapInfo? getClosestBeatmap(IEnumerable<BeatmapInfo> beatmaps, double recommendation)
            => beatmaps.MinBy(b =>
            {
