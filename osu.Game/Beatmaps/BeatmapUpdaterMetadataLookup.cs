...
namespace osu.Game.Beatmaps
{
    ...
    public class BeatmapUpdaterMetadataLookup : IDisposable
    {
        ...
        public void Update(BeatmapSetInfo beatmapSet, bool preferOnlineFetch)
        {
            ...
            var unresolvedBeatmaps = new List<BeatmapInfo>();
            ...
            foreach (var beatmapInfo in beatmapSet.Beatmaps)
            {
                ...
                if (res == null)
                {
                    unresolvedBeatmaps.Add(beatmapInfo);
                    var oldStatus = beatmapInfo.Status;
                    beatmapInfo.ResetOnlineInfo();
                    if (Online.MosuServerEnvironment.IsThirdPartyServer && Online.MosuServerEnvironment.DisableBeatmapStatusOverwrite)
                    {
                        beatmapInfo.Status = oldStatus;
                    }
            ...
            // A beatmap set online ID may be discovered while processing a sibling difficulty.
            // Retry unresolved difficulties once more so API fallback lookups can match within the now-known set.
            if (beatmapSet.OnlineID > 0)
            {
                foreach (var beatmapInfo in unresolvedBeatmaps)
                {
                    if (!tryLookup(beatmapInfo, preferOnlineFetch, out var res) || res == null)
                        continue;
                    ...
                    int unresolvedIndex = lookupResults.FindIndex(r => r == null);
                    if (unresolvedIndex >= 0)
                        lookupResults[unresolvedIndex] = res;
                }
            }

            if (!beatmapSet.Beatmaps.Any(b => b.OnlineID > 0))
            {
                ...
                if (!(Online.MosuServerEnvironment.IsThirdPartyServer && Online.MosuServerEnvironment.DisableBeatmapStatusOverwrite))
                {
                    if (beatmapSet.Status != BeatmapOnlineStatus.LocallyModified)
