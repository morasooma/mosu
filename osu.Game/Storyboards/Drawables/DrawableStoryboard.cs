...
namespace osu.Game.Storyboards.Drawables
{
    public partial class DrawableStoryboard : Container<DrawableStoryboardLayer>
    {
        ...
        public class StoryboardResourceLookupStore : IResourceStore<byte[]>
        {
            ...
            private readonly bool ownsResourceStore;
            private readonly Storyboard storyboard;

            public StoryboardResourceLookupStore(Storyboard storyboard, RealmAccess realm, GameHost host)
            {
                if (storyboard.ResourceStore != null)
                    resourceStore = storyboard.ResourceStore;
                else
                {
                    resourceStore = new RealmFileStore(realm, host.Storage).Store;
                    ownsResourceStore = true;
                }
            ...
            public Task<byte[]> GetAsync(string name, CancellationToken cancellationToken = new CancellationToken())
            {
                ...
                return string.IsNullOrEmpty(storagePath)
                    ...
                    : resourceStore.GetAsync(storagePath, cancellationToken);
            }
            ...
            public Stream? GetStream(string name)
            {
                ...
                return string.IsNullOrEmpty(storagePath)
                    ? null
                    : resourceStore.GetStream(storagePath);
            }

            public IEnumerable<string> GetAvailableResources() =>
                resourceStore.GetAvailableResources();
        }
    }
}
