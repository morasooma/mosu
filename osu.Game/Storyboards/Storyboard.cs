...
namespace osu.Game.Storyboards
{
    public class Storyboard
    {
        ...
        /// <summary>
        /// An optional store containing storyboard resources outside of the realm file store.
        /// This is used by beatmaps which are read directly from an external installation.
        /// </summary>
        internal IResourceStore<byte[]>? ResourceStore { get; set; }
        ...
        public virtual string? GetStoragePathFromStoryboardPath(string path)
        {
            ...
            if (ResourceStore != null)
            {
                if (Path.HasExtension(path))
                    return resourceExists(path) ? path : null;
                ...
                foreach (string ext in SupportedExtensions.IMAGE_EXTENSIONS)
                {
                    ...
                    if (resourceExists(pathWithExtension))
            ...
            bool resourceExists(string resourcePath)
            {
                try
                {
                    using Stream? stream = ResourceStore.GetStream(resourcePath);
                    return stream != null;
                }
                catch
                {
