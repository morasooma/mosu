...
namespace osu.Game.Utils
{
    public static class FormatUtils
    {
        ...
        public static int RoundBPM(double baseBpm, double rate = 1)
        {
            double bpm = baseBpm * rate;

            if (double.IsNaN(bpm))
                return 0;

            if (double.IsPositiveInfinity(bpm))
                return int.MaxValue;

            if (double.IsNegativeInfinity(bpm))
                return int.MinValue;

            return (int)Math.Round(Math.Clamp(bpm, int.MinValue, int.MaxValue));
        }
