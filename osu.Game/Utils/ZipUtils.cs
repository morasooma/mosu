...
namespace osu.Game.Utils
{
    public static class ZipUtils
    {
        ...
        /// <summary>
        /// Checks that an archive is readable and contains at least one beatmap difficulty.
        /// </summary>
        public static bool IsBeatmapArchive(string path)
        {
            if (!File.Exists(path))
                return false;
            ...
            try
            {
                ...
                return archive.Entries.Any(entry => entry.Key?.EndsWith(".osu", StringComparison.OrdinalIgnoreCase) == true);
            }
            catch (Exception)
            {
                return false;
