...
namespace osu.Game.Graphics.Carousel
{
        ...
        where T : notnull
    {
        ...
        private ScheduledDelegate? debounceDelayDelegate;
        ...
        private async Task<IEnumerable<CarouselItem>> performFilter()
        {
            ...
            debounceDelayDelegate?.Cancel();
            debounceDelayDelegate = null;

            if (DebounceDelay > 0)
            {
                log($"Filter operation queued, waiting for {DebounceDelay} ms debounce");

                var debounceCompletion = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);

                debounceDelayDelegate = Scheduler.AddDelayed(() =>
                {
                    debounceDelayDelegate = null;

                    if (cts.IsCancellationRequested)
                        debounceCompletion.TrySetCanceled(cts.Token);
                    else
                        debounceCompletion.TrySetResult(true);
                }, DebounceDelay);
                ...
                {
                    await debounceCompletion.Task.ConfigureAwait(true);
                }
            }
        ...
        /// <summary>
        /// Clears currently displayed panels and forces the next display pass to recreate them.
        /// </summary>
        protected void ResetDisplayedPanels()
        {
            displayedRange = null;
            filterReusesPanels.Invalidate();
        ...
        protected override void Update()
        {
            ...
            foreach (var panel in Scroll.Panels)
            {
                ...
                float depth = c.Item.DepthLayer + normalisedDepth;

                if (panel.Depth != depth)
                    Scroll.Panels.ChangeChildDepth(panel, depth);
                ...
                bool selected = currentSelection?.CarouselItem != null && CheckModelEquality(c.Item, currentSelection.CarouselItem);
                if (c.Selected.Value != selected)
                    c.Selected.Value = selected;

                bool keyboardSelected = c.Item == currentKeyboardSelection?.CarouselItem;
                if (c.KeyboardSelected.Value != keyboardSelected)
                    c.KeyboardSelected.Value = keyboardSelected;

                if (c.Expanded.Value != c.Item.IsExpanded)
        ...
        protected virtual float GetPanelXOffset(Drawable panel)
        {
            ...
            float panelCentreY = (float)(carouselPanel.DrawYPosition - Scroll.Current + panel.DrawHeight / 2);
            float dist = Math.Abs(1f - (panelCentreY + BleedTop) / visibleHalfHeight);
