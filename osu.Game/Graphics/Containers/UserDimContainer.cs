...
namespace osu.Game.Graphics.Containers
{
    ...
    public abstract partial class UserDimContainer : Container
    {
        ...
        private void load(OsuConfigManager config)
        {
            ...
            // The high-dim background blackout (see UpdateVisuals) reads skin performance mode state, so the
            // settings checkboxes must be able to bring a hidden background back without restarting the map.
            SkinPerformanceMode.SettingsChanged += UpdateVisuals;
        }
        ...
        {
            // In skin performance mode a near-black dimmed background is not worth its fullscreen textured pass:
            // the content fades out entirely (alpha 0 also removes its subtree from update/draw traversal) and the
            // plain backbuffer clear shows through instead. IgnoreUserSettings paths (e.g. replay rendering,
            // storyboard-mandated visibility) keep the real image. Skin-perf settings changes take effect on the
            // next dim-related change or gameplay restart, matching the other perf-mode switches.
            bool blackedOut = !IgnoreUserSettings.Value && SkinPerformanceMode.ShouldBlackOutBackground(DimLevel);

            ContentDisplayed = ShowDimContent && !blackedOut;
