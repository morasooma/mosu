...
namespace osu.Game.Graphics.UserInterface
{
        ...
        where T : struct, IEquatable<T>
    {
        ...
        {
            if (SkinPerformanceMode.ShouldSimplifyCounters && this is ISerialisableDrawable)
            {
                FinishTransforms(false, nameof(DisplayedCount));
            ...
            if (!canTransform(currentValue) || !canTransform(newValue))
            {
                FinishTransforms(false, nameof(DisplayedCount));
                DisplayedCount = newValue;
                return;
        ...
        private static bool canTransform(T value) => value switch
        {
