...
namespace osu.Game.Graphics.UserInterface
{
    public partial class FPSCounter : VisibilityContainer, IHasCustomTooltip
    {
        ...
        {
            double activeAimDraw = (aimDrawFPS > 0 && aimDrawFPS < 2000) ? aimDrawFPS : 2000;
            double ratio = Math.Clamp(displayedFpsCount / activeAimDraw, 0, 1);
            counterDrawFPS.Colour = getColour(ratio);
        ...
        private void updateFrameTimeDisplay()
        {
            ...
            double activeAimUpdate = (aimUpdateFPS > 0 && aimUpdateFPS < 2000) ? aimUpdateFPS : 2000;
            double ratio = Math.Clamp((1000 / displayedFrameTime) / activeAimUpdate, 0, 1);
            counterUpdateFrameTime.Colour = getColour(ratio);
        }
        ...
        private ColourInfo getColour(double performanceRatio)
        {
            if (performanceRatio < 0.5f)
                return Interpolation.ValueAt(Math.Max(performanceRatio, 0), colours.Red, colours.Orange2, 0, 0.5);

            return Interpolation.ValueAt(Math.Min(performanceRatio, 0.9), colours.Orange2, colours.Lime0, 0.5, 0.9);
        }
