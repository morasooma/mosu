...
namespace osu.Game.Screens.Ranking
{
    public partial class SoloResultsScreen : ResultsScreen
    {
        ...
        protected override async Task<ScoreInfo[]> FetchScores()
        {
            ...
            var fetchRuleset = Score.Ruleset;

            bool isRelax = Score.Mods.Any(m => m.Acronym == "RX" || m.Acronym == "MRX");
            bool isAutopilot = Score.Mods.Any(m => m.Acronym == "AP");
            ...
            foreach (var clonedScore in clonedScores)
            {
                ...
                else
                {
                    ...
                    bool presentedLocalUserScoreIsBetter = presentingLocalUserScore && clonedScore.UserID == api.LocalUser.Value.OnlineID
                                                                            && clonedScore.GetLeaderboardScore() < Score.GetLeaderboardScore();
