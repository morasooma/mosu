...
namespace osu.Game.Screens.Ranking
{
    public partial class ScorePanel : CompositeDrawable, IStateful<PanelState>
    {
        ...
        private ColourInfo expanded_top_layer_colour
        {
            get
            {
                if (OverlayColourProvider.IsDarkTheme)
                    return ColourInfo.GradientVertical(Color4Extensions.FromHex("#1a1a1a"), Color4Extensions.FromHex("#0d0d0d"));
                if (OverlayColourProvider.IsLightTheme)
                    return ColourInfo.GradientVertical(Color4Extensions.FromHex("#e8e8e8"), Color4Extensions.FromHex("#dedede"));
                return ColourInfo.GradientVertical(Color4Extensions.FromHex("#444"), Color4Extensions.FromHex("#333"));
            }
        }

        private ColourInfo expanded_middle_layer_colour
        {
            get
            {
                if (OverlayColourProvider.IsDarkTheme)
                    return ColourInfo.GradientVertical(Color4Extensions.FromHex("#222"), Color4Extensions.FromHex("#0d0d0d"));
                if (OverlayColourProvider.IsLightTheme)
                    return ColourInfo.GradientVertical(Color4Extensions.FromHex("#ebebeb"), Color4Extensions.FromHex("#e0e0e0"));
                return ColourInfo.GradientVertical(Color4Extensions.FromHex("#555"), Color4Extensions.FromHex("#333"));
            }
        }

        private Color4 contracted_top_layer_colour
        {
            get
            {
                if (OverlayColourProvider.IsDarkTheme) return Color4Extensions.FromHex("#0a0a0a");
                if (OverlayColourProvider.IsLightTheme) return Color4Extensions.FromHex("#e0e0e0");
                return Color4Extensions.FromHex("#353535");
            }
        }

        private Color4 contracted_middle_layer_colour
        {
            get
            {
                if (OverlayColourProvider.IsDarkTheme) return Color4Extensions.FromHex("#0a0a0a");
