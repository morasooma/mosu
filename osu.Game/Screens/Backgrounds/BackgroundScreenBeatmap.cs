...
namespace osu.Game.Screens.Backgrounds
{
    public partial class BackgroundScreenBeatmap : BackgroundScreen
    {
        ...
        private void load()
        {
            var initialBackground = CreateBackgroundDrawable(beatmap);
            LoadComponent(initialBackground);
            switchBackground(initialBackground);
        }
        ...
        protected virtual BeatmapBackground CreateBackgroundDrawable(WorkingBeatmap beatmap) => new BeatmapBackground(beatmap);

        protected virtual bool CanReuseCurrentBackground(WorkingBeatmap newBeatmap)
        {
            var currentBeatmap = (Background as BeatmapBackground)?.Beatmap;

            if (currentBeatmap == null || newBeatmap == null)
                return false;

            if (StablePathManager.IsStableBeatmap(currentBeatmap.BeatmapInfo.ID) || StablePathManager.IsStableBeatmap(newBeatmap.BeatmapInfo.ID))
            {
                if (currentBeatmap is StableWorkingBeatmap currentStable && newBeatmap is StableWorkingBeatmap newStable)
                    return currentStable.BackgroundFilePath == newStable.BackgroundFilePath;

                return false;
            }

            return currentBeatmap.BeatmapInfo.BackgroundEquals(newBeatmap.BeatmapInfo);
        }

        protected void ReloadBackground() => Schedule(() => queueBackgroundLoad(force: true));

        private void queueBackgroundLoad(bool force = false)
        {
            if (!force && CanReuseCurrentBackground(beatmap))
                return;

            cancellationSource?.Cancel();
            LoadComponentAsync(CreateBackgroundDrawable(beatmap), switchBackground, (cancellationSource = new CancellationTokenSource()).Token);
        }

        private void switchBackground(BeatmapBackground b)
        {
            if (b.Beatmap != beatmap)
            {
                b.Expire();
                return;
            }
