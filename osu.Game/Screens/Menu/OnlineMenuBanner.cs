...
namespace osu.Game.Screens.Menu
{
    public partial class OnlineMenuBanner : VisibilityContainer
    {
        ...
        {
            if (!FetchOnlineContent || Online.MosuServerEnvironment.IsThirdPartyServer)
                return;
        ...
        private void loadNewImages(ValueChangedEvent<APIMenuContent> images)
        {
            ...
            if (images.NewValue?.Images == null || images.NewValue.Images.Length == 0)
            {
                displayIndex = -1;
                return;
            }
        ...
        public partial class MenuImage : OsuClickableContainer
        {
            ...
            private void load(LargeTextureStore textureStore, OsuGame? game)
            {
                ...
                if (texture != null)
                {
                    if (Image.Image.Contains(@"@2x"))
                        texture.ScaleAdjust *= 2;

                    // Restrict maximum height to prevent huge user-uploaded banners from covering the screen
                    const float max_height = 120f;
                    if (texture.DisplayHeight > max_height)
                        texture.ScaleAdjust *= (float)(texture.DisplayHeight / max_height);
                }
