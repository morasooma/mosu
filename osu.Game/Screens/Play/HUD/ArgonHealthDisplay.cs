...
namespace osu.Game.Screens.Play.HUD
{
    public partial class ArgonHealthDisplay : HealthDisplay, ISerialisableDrawable
    {
        ...
        protected override void Update()
        {
            ...
            {
                healthBarValue = Current.Value;
                glowBarValue = Current.Value;
            }
            else
            {
            ...
            mainBar.Alpha = SkinPerformanceMode.ShouldSimplifyHud
                ? Current.Value > 0 ? 1 : 0
                : (float)Interpolation.DampContinuously(mainBar.Alpha, Current.Value > 0 ? 1 : 0, 40, Time.Elapsed);
            glowBar.Alpha = SkinPerformanceMode.ShouldSimplifyHud
                ? 0
                : (float)Interpolation.DampContinuously(glowBar.Alpha, glowBarValue > 0 ? 1 : 0, 40, Time.Elapsed);
        ...
        protected override void FinishInitialAnimation(double value)
        {
            ...
            if (SkinPerformanceMode.ShouldSimplifyHud)
            {
                healthBarValue = glowBarValue = value;
                updatePathProgress();
                return;
            }
        ...
        private void updatePathProgress()
        {
            ...
            float mainEnd = (float)healthBarValue;

            if (Math.Abs(mainBar.ProgressRange.Y - mainEnd) > epsilon)
                mainBar.ProgressRange = new Vector2(0f, mainEnd);

            float glowStart = (float)healthBarValue;
            float glowEnd = (float)Math.Max(glowBarValue, healthBarValue);
            Vector2 glowRange = new Vector2(glowStart, glowEnd);

            if (Math.Abs(glowBar.ProgressRange.X - glowRange.X) > epsilon || Math.Abs(glowBar.ProgressRange.Y - glowRange.Y) > epsilon)
                glowBar.ProgressRange = glowRange;
        }
