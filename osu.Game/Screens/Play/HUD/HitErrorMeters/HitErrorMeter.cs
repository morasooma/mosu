...
namespace osu.Game.Screens.Play.HUD.HitErrorMeters
{
    public abstract partial class HitErrorMeter : CompositeDrawable, ISerialisableDrawable
    {
        ...
        private bool disabledForRuleset;

        [BackgroundDependencyLoader(true)]
        private void load(DrawableRuleset drawableRuleset)
        {
            HitWindows = drawableRuleset?.FirstAvailableVisualHitWindows ?? HitWindows.Empty;

            if (drawableRuleset?.Ruleset.ShortName == "dodge")
            {
                disabledForRuleset = true;
                Alpha = 0;
                AlwaysPresent = false;
                return;
            }
        ...
        {
            if (!j.IgnoreForHitErrorMeter)
                Schedule(() => OnNewJudgement(j));
        }
