...
namespace osu.Game.Screens.Play
{
    public partial class ComboEffects : CompositeDrawable
    {
        ...
        private static readonly SampleInfo combo_break_info = new SampleInfo("Gameplay/combobreak");
        ...
        {
            InternalChild = comboBreakSample = new SkinnableSound(combo_break_info);
        ...
        private void onComboChange(ValueChangedEvent<int> combo)
        {
            ...
            if (combo.NewValue == 0 && (combo.OldValue > 20 || (alwaysPlayFirst.Value && firstBreakTime == null)))
            {
                ...
                {
                    // During render, SamplePlaybackDisabled suppresses SkinnableSound.Play().
                    // Route through the render hitsound renderer to capture the combobreak sound.
                    renderGameplaySampleTriggerRecorder?.RecordTrigger(gameplayClock.CurrentTime);
                    renderGameplayHitsoundRenderer?.PlaySamples(new[] { combo_break_info });
                    return;
                }
