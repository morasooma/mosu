...
namespace osu.Game.Screens.Edit
{
    ...
    public partial class EditorLoader : ScreenWithBeatmapBackground
    {
        ...
        public void ScheduleSwitchToNewDifficulty(
            ...
            bool createInSeparateRulesetSet = false)
            => scheduleDifficultySwitch(() =>
            {
                try
                {
                    ...
                    if (createCopy)
                        return beatmapManager.CopyExistingDifficulty(targetBeatmapSet, referenceWorkingBeatmap);

                    return createInSeparateRulesetSet
                        ? beatmapManager.CreateNewDifficultyInSeparateRulesetSet(targetBeatmapSet, referenceWorkingBeatmap, rulesetInfo, api.LocalUser.Value)
        ...
        public void ScheduleExtractRulesetDifficulties(BeatmapInfo referenceBeatmapInfo, RulesetInfo rulesetInfo, EditorState editorState)
            => scheduleDifficultySwitch(() =>
            {
                try
                {
                    var sourceBeatmapSet = beatmapManager.QueryBeatmap(b => b.ID == referenceBeatmapInfo.ID).AsNonNull().BeatmapSet.AsNonNull();
                    return beatmapManager.ExtractRulesetDifficultiesToSeparateSet(sourceBeatmapSet, rulesetInfo, api.LocalUser.Value);
                }
                catch (Exception ex)
                {
                    Logger.Error(ex, ex.Message);
