...
namespace osu.Game.Screens.Edit
{
    ...
    public partial class Editor : OsuScreen, IKeyBindingHandler<GlobalAction>, IKeyBindingHandler<PlatformAction>, IBeatSnapProvider, ISamplePlaybackDisabler, IBeatSyncProvider
    {
            ...
            try
            {
                ...
                switch (e.NewValue)
                {
                    ...
                    case EditorScreenMode.Design:
                        currentScreen = editorBeatmap.BeatmapInfo.Ruleset.CreateInstance() is IEditorDesignScreenProvider designScreenProvider
                            ? designScreenProvider.CreateDesignScreen()
                            : new DesignScreen();
                        break;
        ...
        private EditorMenuItem createDifficultyCreationMenu()
        {
            ...
            BeatmapSetInfo currentSet = Beatmap.Value.BeatmapSetInfo;
            RulesetInfo dodgeRuleset = currentSet.Beatmaps.FirstOrDefault(beatmap => beatmap.Ruleset.OnlineID == 10)?.Ruleset;

            if (dodgeRuleset != null && currentSet.Beatmaps.Any(beatmap => beatmap.Ruleset.OnlineID != 10))
            {
                rulesetItems.Add(new OsuMenuItemSpacer());
                rulesetItems.Add(new EditorMenuItem(
                    EditorDialogsStrings.ExtractRulesetDifficulties(dodgeRuleset.Name),
                    MenuItemType.Standard,
                    () => extractRulesetDifficulties(dodgeRuleset)));
            }
        ...
        private void extractRulesetDifficulties(RulesetInfo rulesetInfo)
        {
            if (HasUnsavedChanges)
            {
                dialogOverlay.Push(new SaveRequiredPopupDialog(() => attemptMutationOperation(() =>
                {
                    ...
                    extractRulesetDifficulties(rulesetInfo);
                    return true;
                })));
            ...
            int difficultyCount = Beatmap.Value.BeatmapSetInfo.Beatmaps.Count(beatmap => beatmap.Ruleset.OnlineID == rulesetInfo.OnlineID);

            dialogOverlay.Push(new ExtractRulesetDifficultiesDialog(rulesetInfo.Name, difficultyCount, () =>
            {
                switchingDifficulty = true;
                loader?.ScheduleExtractRulesetDifficulties(editorBeatmap.BeatmapInfo, rulesetInfo, GetState(rulesetInfo));
            }));
        }
        ...
        protected internal void CreateNewDifficulty(RulesetInfo rulesetInfo)
        {
            ...
            bool creatingFirstDodgeDifficulty =
                rulesetInfo.OnlineID == 10
                && !Beatmap.Value.BeatmapSetInfo.Beatmaps.Any(beatmap => beatmap.Ruleset.OnlineID == 10);

            if (creatingFirstDodgeDifficulty)
            {
                if (beatmapManager.FindSeparateRulesetBeatmapSet(Beatmap.Value.BeatmapSetInfo, rulesetInfo, api.LocalUser.Value) != null)
                {
                    switchToNewDifficulty(rulesetInfo, false, createInSeparateRulesetSet: true);
                    return;
                }

                dialogOverlay.Push(new CreateSeparateRulesetBeatmapSetDialog(
                    rulesetInfo.Name,
                    () => switchToNewDifficulty(rulesetInfo, false, createInSeparateRulesetSet: true),
        ...
        private void switchToNewDifficulty(RulesetInfo rulesetInfo, bool createCopy, bool createInSeparateRulesetSet = false)
        {
            switchingDifficulty = true;
            loader?.ScheduleSwitchToNewDifficulty(editorBeatmap.BeatmapInfo, rulesetInfo, createCopy, GetState(rulesetInfo), createInSeparateRulesetSet);
        }
