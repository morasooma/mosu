                    ...
                    if (timeline.FindSnappedPositionAndTime(e.ScreenSpaceMousePosition).Time is double time)
                    {
                        switch (hitObject)
                        {
                            ...
                            case IHasDuration endTimeHitObject:
                                ...
                                {
                                    // EndTime is not necessarily StartTime + Duration for every object.
                                    // For example, an emitter may include an emission window before the
                                    // projectile travel duration. Let the object translate the requested
                                    // timeline edge into its own duration representation.
                                    if (obj is IEditorTimelineEndTimeAdjustable adjustable)
                                        adjustable.SetEditorTimelineEndTime(snappedTime);
                                    else
