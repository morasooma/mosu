...
namespace osu.Game.Screens.Edit
{
    public class EditorBeatmapProcessor : IBeatmapProcessor
    {
        ...
        private readonly IEditorBreakProcessor? rulesetBreakProcessor;
        ...
        public EditorBeatmapProcessor(EditorBeatmap beatmap, Ruleset ruleset)
        {
            ...
            rulesetBreakProcessor = rulesetBeatmapProcessor as IEditorBreakProcessor;
        }
        ...
        public void PostProcess()
        {
            ...
            autoGenerateBreaks();
            rulesetBreakProcessor?.PostProcessAutomaticBreaks(Beatmap);
            ensureNewComboAfterBreaks();
        }

        private void autoGenerateBreaks()
        {
            var breakRelevantHitObjects = (rulesetBreakProcessor?.GetBreakRelevantHitObjects() ?? Beatmap.HitObjects).ToList();
            var objectDuration = breakRelevantHitObjects.Select(ho => (ho.StartTime - ((ho as IHasTimePreempt)?.TimePreempt ?? 0), ho.GetEndTime())).ToHashSet();

            if (rulesetBreakProcessor?.AlwaysRegenerateAutomaticBreaks != true && objectDuration.SetEquals(objectDurationCache))
                return;
            ...
            double? firstObjectStartTime = breakRelevantHitObjects.FirstOrDefault()?.StartTime;
            double? lastObjectEndTime = breakRelevantHitObjects.Count == 0
                ? null
                : breakRelevantHitObjects.Max(hitObject => hitObject.GetEndTime());

            foreach (var manualBreak in Beatmap.Breaks.ToList())
            {
                if (firstObjectStartTime == null
                    || manualBreak.EndTime <= firstObjectStartTime.Value
                    || manualBreak.StartTime >= lastObjectEndTime!.Value
                    || breakRelevantHitObjects.Any(ho => ho.StartTime <= manualBreak.EndTime && ho.GetEndTime() >= manualBreak.StartTime))
                {
            ...
            for (int i = 1; i < breakRelevantHitObjects.Count; ++i)
            {
                var previousObject = breakRelevantHitObjects[i - 1];
                var nextObject = breakRelevantHitObjects[i];
