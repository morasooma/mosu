    ...
    public static class FilterQueryParser
    {
        ...
        private static bool tryParseKeywordCriteria(FilterCriteria criteria, string key, string value, Operator op)
        {
            switch (key)
            {
                ...
                case "pp":
                    return TryUpdateCriteriaRange(ref criteria.PerformancePoints, op, value, 0);
                ...
                case "objects":
                case "obj":
                    return TryUpdateCriteriaRange(ref criteria.TotalObjects, op, value);

                case "status":
                    if (op == Operator.Equal && (value.Equals("e", StringComparison.OrdinalIgnoreCase) || value.Equals("exclusive", StringComparison.OrdinalIgnoreCase)))
                    {
                        criteria.ServerExclusive = true;
                        return true;
                    }
                ...
                case "key":
                case "keys":
                    return tryUpdateManiaKeyFilter(criteria, op, value);
        ...
        private static bool tryUpdateManiaKeyFilter(FilterCriteria criteria, Operator op, string value)
        {
            ...
            {
                criteria.ManiaKeyCounts.Clear();
                return true;
            }

            string[] splitValues = value.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
            ...
            {
                if (!tryParseInt(splitValue, out int parsedValue))
                    return false;

                criteria.ManiaKeyCounts.Add(parsedValue);
            }
        ...
        private static bool tryUpdateCriteriaRange(ref FilterCriteria.OptionalRange<float> range, Operator op, float value, float tolerance = 0.05f)
        {
            ...
            switch (op)
            {
                ...
                case Operator.Approximate:
                    range.Min = value * 0.9f;
                    range.Max = value * 1.1f;
                    range.IsLowerInclusive = range.IsUpperInclusive = true;
                    break;
        ...
        /// <summary>
        /// Attempts to parse a keyword filter of type <see cref="int"/>
        /// from the specified <paramref name="op"/> and <paramref name="val"/>.
        /// Supports <see cref="Operator.Approximate"/> which produces a ±10% range.
        /// </summary>
        public static bool TryUpdateCriteriaRange(ref FilterCriteria.OptionalRange<int> range, Operator op, string val)
            => tryParseInt(val, out int value) && tryUpdateCriteriaRangeInt(ref range, op, value);

        private static bool tryUpdateCriteriaRangeInt(ref FilterCriteria.OptionalRange<int> range, Operator op, int value)
        {
            range.InvertRange = false;
            ...
            switch (op)
            {
                ...
                case Operator.Approximate:
                    range.Min = (int)Math.Floor(value * 0.9);
                    range.Max = (int)Math.Ceiling(value * 1.1);
                    range.IsLowerInclusive = range.IsUpperInclusive = true;
                    break;
        ...
        private static bool tryUpdateLengthRange(FilterCriteria criteria, Operator op, string val)
        {
            ...
            // For ~ (Approximate), use ±10% of the total length in milliseconds.
            if (op == Operator.Approximate)
                return tryUpdateCriteriaRange(ref criteria.Length, Operator.GreaterOrEqual, totalLength * 0.9, 0)
                       && tryUpdateCriteriaRange(ref criteria.Length, Operator.LessOrEqual, totalLength * 1.1, 0);
