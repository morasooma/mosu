    ...
    public partial class NoResultsPlaceholder : VisibilityContainer
    {
        ...
        private void load()
        {
            ...
            themeColour.BindValueChanged(_ =>
            {
                ...
                textFlow.Colour = colourProvider.Content2;
        ...
        private void updateText()
        {
            ...
            if (filter?.Ruleset?.OnlineID == 10 &&
                beatmaps.QueryBeatmap($@"{nameof(BeatmapInfo.Ruleset)}.{nameof(RulesetInfo.OnlineID)} == $0", 10) == null)
            {
                textFlow.AddParagraph(SongSelectStrings.NoDodgeBeatmapsDescription);
                addBulletPoint();
                textFlow.AddLink(SongSelectStrings.BrowseDodgeBeatmaps, () =>
                {
                    beatmapListingOverlay?.ShowForRuleset(filter.Ruleset);
                });
                return;
            }
            ...
            else
            {
                ...
                if (filter?.Ruleset?.OnlineID is not 0 and not 10 && filter?.AllowConvertedBeatmaps == false)
                {
                    addBulletPoint();
                    textFlow.AddText("Try ");
