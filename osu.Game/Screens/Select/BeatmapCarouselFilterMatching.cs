...
namespace osu.Game.Screens.Select
{
    public class BeatmapCarouselFilterMatching : ICarouselFilter
    {
        ...
        public static bool CheckCriteriaMatch(BeatmapInfo beatmap, FilterCriteria criteria)
        {
            ...
            match &= !criteria.StarDifficulty.HasFilter || criteria.StarDifficulty.IsInRange(getStarRating(beatmap, criteria).FloorToDecimalDigits(2));
            ...
            match &= !criteria.TotalObjects.HasFilter || (beatmap.TotalObjectCount >= 0 && criteria.TotalObjects.IsInRange(beatmap.TotalObjectCount));
            match &= !criteria.PerformancePoints.HasFilter || checkPerformancePoints(beatmap, criteria);

            if (criteria.ServerExclusive)
            {
                const int server_exclusive_id_threshold = 2_000_000_000;
                match &= beatmap.OnlineID >= server_exclusive_id_threshold
            ...
            match &= !criteria.UserStarDifficulty.HasFilter || criteria.UserStarDifficulty.IsInRange(getStarRating(beatmap, criteria));

            if (!match) return false;

            if (criteria.ManiaKeyCounts.Count > 0)
            {
                var maniaSummary = criteria.Ruleset == null ? null : ManiaBeatmapSummary.Create(beatmap, criteria.Ruleset, criteria.Mods ?? Array.Empty<Rulesets.Mods.Mod>());

                match &= maniaSummary != null && criteria.ManiaKeyCounts.Contains(maniaSummary.Value.KeyCount);
            }
        ...
        private static bool checkPerformancePoints(BeatmapInfo beatmap, FilterCriteria criteria)
        {
            double pp = isRelax(criteria)
                ? ForkDataStore.Instance?.GetRelaxPP(beatmap.ID) ?? -1
                : ForkDataStore.Instance?.GetPP(beatmap.ID) ?? -1;

            return pp > 0 && criteria.PerformancePoints.IsInRange(pp);
        }

        private static double getStarRating(BeatmapInfo beatmap, FilterCriteria criteria)
        {
            if (!isRelax(criteria))
                return beatmap.StarRating;

            return ForkDataStore.Instance?.GetRelaxStarRating(beatmap.ID) ?? -1;
        }

        private static bool isRelax(FilterCriteria criteria) => criteria.Mods?.Any(m => m is ModRelax || m.Acronym == "MRX") == true;
    }
}
