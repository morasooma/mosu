...
namespace osu.Game.Screens.Select
{
    ...
    public partial class LegacyLeaderboard : CompositeDrawable
    {
        ...
        private Bindable<ForkSongSelectStyle> legacyStyle = null!;
        ...
        protected override void LoadComplete()
        {
            ...
            legacyStyle = config.GetBindable<ForkSongSelectStyle>(OsuSetting.ForkSongSelectStyle);

            scores.BindTo(leaderboardManager.Scores);
            scores.BindValueChanged(_ => updateScores());

            // Stable-compatible servers can resolve server-exclusive maps by checksum even when
            // their local OnlineID is missing. Start on Global there and never change the user's
            // selected scope just because another beatmap was selected.
            Scope.Value = GetInitialScope(api.IsLoggedIn, MosuServerEnvironment.UsesStableProtocol);
            ...
            {
                if (GetSpecialLeaderboardMode(change.OldValue) != GetSpecialLeaderboardMode(change.NewValue))
                    refetch();
            });
            Scope.BindValueChanged(_ => refetch(forceRefresh: true));
            legacyStyle.BindValueChanged(active =>
            {
                if (active.NewValue.UsesStableStyle())
                    refetch();
            }, true);
        ...
        internal static BeatmapLeaderboardScope GetInitialScope(bool isLoggedIn, bool usesStableProtocol)
            => isLoggedIn || usesStableProtocol ? BeatmapLeaderboardScope.Global : BeatmapLeaderboardScope.Local;
        ...
        private void refetch(bool forceRefresh = false)
        {
            ...
            if (!legacyStyle.Value.UsesStableStyle())
                return;

            var beatmapInfo = beatmap.Value?.BeatmapInfo;
            var fetchRuleset = GetLeaderboardRuleset(ruleset.Value ?? beatmapInfo?.Ruleset, mods.Value);

            lastFetchedCriteria = new LeaderboardCriteria(beatmapInfo, fetchRuleset, Scope.Value, null);
            reclaimedForeignFetch = false;
            leaderboardManager.FetchWithCriteria(lastFetchedCriteria, forceRefresh);
        }

        internal static int GetSpecialLeaderboardMode(IReadOnlyList<Mod> selectedMods)
        {
        ...
        internal static RulesetInfo? GetLeaderboardRuleset(RulesetInfo? currentRuleset, IReadOnlyList<Mod> selectedMods)
        {
            if (currentRuleset == null)
                return null;

            bool isRelax = selectedMods.Any(m => m.Acronym is "RX" or "MRX");
            bool isAutopilot = selectedMods.Any(m => m.Acronym == "AP");
            ...
            if (isRelax)
            {
                ...
                if (!MosuServerEnvironment.OnlyOsuRelax)
                {
                    ...
                    if (currentRuleset.ShortName == RulesetInfo.CATCH_MODE_SHORTNAME)
                        return currentRuleset.CreateSpecialRuleset(RulesetInfo.CATCH_RELAX_MODE_SHORTNAME, RulesetInfo.CATCH_RELAX_ONLINE_ID);
                }
            }
            else if (isAutopilot && !MosuServerEnvironment.OnlyOsuRelax && currentRuleset.ShortName == RulesetInfo.OSU_MODE_SHORTNAME)
                return currentRuleset.CreateSpecialRuleset(RulesetInfo.OSU_AUTOPILOT_MODE_SHORTNAME, RulesetInfo.OSU_AUTOPILOT_ONLINE_ID);
        ...
        private void updateScores()
        {
            ...
            if (legacyStyle.Value.UsesStableStyle() && lastFetchedCriteria != null && leaderboardManager.CurrentCriteria != null
