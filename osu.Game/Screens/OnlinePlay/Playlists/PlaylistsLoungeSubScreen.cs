...
namespace osu.Game.Screens.OnlinePlay.Playlists
{
    public partial class PlaylistsLoungeSubScreen : LoungeSubScreen
    {
        ...
        protected override LoungeFilterCriteria CreateFilterCriteria()
        {
            ...
            // Playlist rooms can contain maps from different rulesets, so filtering the lounge
            // by the currently selected global ruleset hides valid rooms from other players.
        ...
        public override void Delete(Room room)
        {
            Debug.Assert(room.RoomID != null);

            var request = new DeletePlaylistRequest(room.RoomID.Value);
            request.Success += RefreshRooms;
            api.Queue(request);
        }
