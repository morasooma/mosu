...
namespace osu.Game.Screens.OnlinePlay.Matchmaking.RankedPlay
{
    public partial class GameplayWarmupScreen : RankedPlaySubScreen
    {
        ...
        private OsuSpriteText rouletteText = null!;
        ...
        private void load()
        {
            ...
            [
                ...
                rouletteText = new OsuSpriteText
                {
                    Anchor = Anchor.BottomCentre,
                    Origin = Anchor.BottomCentre,
                    Y = -70,
                    Alpha = 0,
                    Font = OsuFont.GetFont(size: 38, weight: FontWeight.Bold),
                }
            ];
        }
        ...
        protected override void LoadComplete()
        {
            ...
            var state = (RankedPlayRoomState?)Client.Room.MatchState;

            if (state?.RandomMods == true && !string.IsNullOrEmpty(state.RandomModAcronym))
                runModRoulette(state.RandomModAcronym, state.AvailableRandomMods);
            else
                Client.ChangeState(MultiplayerUserState.Ready).FireAndForget();
        }

        private void runModRoulette(string selectedMod, string[] availableMods)
        {
            if (availableMods.Length == 0)
                availableMods = [selectedMod];

            rouletteText.Text = "Random mod: AA";
            rouletteText.FadeIn(200);
            ...
            {
                int index = i;
                Scheduler.AddDelayed(() => rouletteText.Text = $"Random mod: {availableMods[index % availableMods.Length]}", i * 75);
            }

            Scheduler.AddDelayed(() =>
            {
                rouletteText.Text = $"Random mod: {selectedMod}";
                rouletteText.ScaleTo(1.2f, 180, Easing.OutBack).Then().ScaleTo(1, 220, Easing.OutQuint);
                Client.ChangeState(MultiplayerUserState.Ready).FireAndForget();
            }, spins * 75);
        }
