...
namespace osu.Game.Screens.OnlinePlay.Matchmaking.Queue
{
    ...
    public partial class QueueController : Component
    {
        ...
        /// <param name="randomMods">Whether to use the isolated random-mods queue.</param>
        public void JoinQueue(MatchmakingPool pool, bool randomMods = false)
        {
            lastDuelUser = null;
            lastDuelPool = null;
        ...
        public void RejoinQueue()
        {
            if (lastDuelUser != null && lastDuelPool != null)
                ...
                JoinQueue(SelectedPool.Value, lastRandomMods);
        }
        ...
        private void onMatchmakingRoomReady(long roomId, string password) => Scheduler.Add(async () =>
        {
            CurrentState.Value = ScreenQueue.MatchmakingScreenState.InRoom;

            try
            {
                await client.JoinRoom(new Room { RoomID = roomId }, password).ConfigureAwait(true);
            }
            catch (Exception ex)
            {
                Logger.Log($"{nameof(QueueController)}: JoinRoom for matchmaking room {roomId} failed: {ex.Message}", LoggingTarget.Runtime, LogLevel.Important);
                CurrentState.Value = ScreenQueue.MatchmakingScreenState.Idle;
            }
        });
