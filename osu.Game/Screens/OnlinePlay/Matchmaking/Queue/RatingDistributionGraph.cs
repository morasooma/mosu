...
namespace osu.Game.Screens.OnlinePlay.Matchmaking.Queue
{
    public partial class RatingDistributionGraph : CompositeDrawable, IHasCustomTooltip<RatingDistributionGraph.RatingDistributionGraphTooltipData>
    {
        ...
        public void SetData((int x, int y)[] data, int? userRating)
        {
            ...
            {
                int centre = userRating ?? data.Select(d => d.x).DefaultIfEmpty(1500).First();
                xRange.min = (int)floorWithFactor(centre - xDivisionStep * 0.5, xDivisionStep);
                xRange.max = xRange.min + xDivisionStep;
            }

            if (xRange.step <= 0)
                xRange.step = xDivisionStep;
            ...
            yRange = (
                0,
                Math.Max(1, (int)ceilingWithFactor(yMax, significantOfNumber(Math.Max(yMax, 1))))
            );
        ...
        private void updateGraph() => Scheduler.AddOnce(() =>
        {
            ...
            else
            {
                ...
                float p = totalPlayers > 0 ? (float)countPlayersBelow / totalPlayers : 0;
        ...
        {
            float xDenom = xRange.max - xRange.min;
            float yDenom = yRange.max - yRange.min;

            float xPos = xDenom > 0 ? ((float)x - xRange.min) / xDenom : 0.5f;
            float yPos = yDenom > 0 ? 1 - ((float)y - yRange.min) / yDenom : 1;
