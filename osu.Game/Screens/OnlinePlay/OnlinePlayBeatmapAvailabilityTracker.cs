    ...
    public abstract partial class OnlinePlayBeatmapAvailabilityTracker : CompositeComponent
    {
        ...
        protected override void LoadComplete()
        {
            ...
            PlaylistItem.BindValueChanged(item =>
            {
                ...
                {
                    var currentItem = PlaylistItem.Value;
                    var beatmap = task.GetResultSafely() ?? currentItem?.Beatmap as APIBeatmap;

                    if (beatmap != null && currentItem?.Beatmap.OnlineID == beatmap.OnlineID)
                        startTracking(beatmap, currentItem.Beatmap.MD5Hash);
        ...
        private void startTracking(APIBeatmap beatmap, string expectedChecksum)
        {
            Debug.Assert(beatmap.BeatmapSet != null);

            if (string.IsNullOrEmpty(expectedChecksum))
                expectedChecksum = beatmap.MD5Hash;
            ...
            {
                if (StablePathManager.IsAvailableLocally(beatmap.OnlineID, expectedChecksum))
                {
            ...
            {
                var beatmaps = realm.Realm.All<BeatmapInfo>().NotDeleted();

                return string.IsNullOrEmpty(expectedChecksum)
                    ? beatmaps.ForOnlineId(beatmap.OnlineID)
                    : beatmaps.Filter($@"{nameof(BeatmapInfo.MD5Hash)} == $0", expectedChecksum);
            }
        }
