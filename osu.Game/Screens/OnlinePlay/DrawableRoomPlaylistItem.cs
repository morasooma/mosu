        ...
        private void refresh()
        {
            ...
            if (difficultyIconContainer != null)
            {
                if (beatmap != null)
                {
                    ...
                    IBeatmapSetInfo? beatmapSet = beatmap.BeatmapSet;

                    if (beatmapSet is IBeatmapSetOnlineInfo onlineInfo)
                    {
                        children.Add(thumbnail = new BeatmapCardThumbnail(beatmapSet, onlineInfo)
                        {
                    ...
                    children.Add(new DifficultyIcon(beatmap, ruleset, requiredMods)
                    {
                        Size = new Vector2(24),
                    ...
                    difficultyIconContainer.Children = children;
                }
                else
                {
                    thumbnail = null;
            ...
            private void stateChanged(ValueChangedEvent<DownloadState> state)
            {
                switch (state.NewValue)
                {
                    ...
                    case DownloadState.LocallyAvailable:
                        ...
                        if (!StablePathManager.IsAvailableLocally(beatmap.OnlineID, beatmap.MD5Hash)
                            && beatmapManager.QueryBeatmap(b => b.MD5Hash == beatmap.MD5Hash) == null)
