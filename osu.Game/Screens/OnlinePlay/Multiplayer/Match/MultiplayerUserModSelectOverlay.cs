...
namespace osu.Game.Screens.OnlinePlay.Multiplayer.Match
{
    public partial class MultiplayerUserModSelectOverlay : UserModSelectOverlay
    {
        ...
        private void onSelectedModsChanged(ValueChangedEvent<IReadOnlyList<Mod>> mods)
        {
            ...
            // Perform check: only allow select/send of mods that are actually allowed in the room.
            ...
            var allowedNewMods = mods.NewValue.Where(m => allowedMods.Any(a => a.GetType() == m.GetType())).ToList();
            if (!allowedNewMods.SequenceEqual(mods.NewValue))
            {
                SelectedMods.Value = allowedNewMods;
                return;
            }
            ...
            modSettingChangeTracker.SettingChanged += _ =>
            {
                ...
                debouncedModSettingsUpdate = Scheduler.AddDelayed(() =>
                {
                    ...
                    var allowedCurrentMods = SelectedMods.Value.Where(m => allowedMods.Any(a => a.GetType() == m.GetType())).ToList();
                    client.ChangeUserMods(allowedCurrentMods).FireAndForget();
                }, 500);
            };
        }
