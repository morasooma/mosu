...
namespace osu.Game.Rulesets.Osu.Tests
{
    public partial class TestSceneStartTimeOrderedHitPolicy : RateAdjustedBeatmapTestScene
    {
        ...
        [Test]
        public void TestCrashScenario()
        {
            var hitObjects = new List<OsuHitObject>
            {
                new CustomPreemptSlider
                {
                    StartTime = 520,
                    CustomPreempt = 25,
                ...
                new CustomPreemptHitCircle { StartTime = 550, CustomPreempt = 70, Position = Vector2.Zero },
                new CustomPreemptHitCircle { StartTime = 750, CustomPreempt = 300, Position = Vector2.Zero },
                new CustomPreemptHitCircle { StartTime = 950, CustomPreempt = 500, Position = Vector2.Zero }
            };

            performTest(hitObjects, new List<ReplayFrame>
            {
                new OsuReplayFrame { Time = 470, Position = Vector2.Zero, Actions = { OsuAction.LeftButton } },
                new OsuReplayFrame { Time = 480, Position = Vector2.Zero },
                new OsuReplayFrame { Time = 500, Position = Vector2.Zero, Actions = { OsuAction.RightButton } },
                new OsuReplayFrame { Time = 510, Position = Vector2.Zero }
            });
        }

        private class CustomPreemptSlider : TestSlider
        {
            public double CustomPreempt { get; set; }

            protected override void ApplyDefaultsToSelf(osu.Game.Beatmaps.ControlPoints.ControlPointInfo controlPointInfo, IBeatmapDifficultyInfo difficulty)
            {
                base.ApplyDefaultsToSelf(controlPointInfo, difficulty);
                TimePreempt = CustomPreempt;
            }
        }

        [Test]
        public void TestCrashWithSeeking()
        {
            var hitObjects = new List<OsuHitObject>
            {
                new CustomPreemptHitCircle { StartTime = 800, CustomPreempt = 15, Position = Vector2.Zero },
                new CustomPreemptHitCircle { StartTime = 1000, CustomPreempt = 300, Position = Vector2.Zero },
                new CustomPreemptHitCircle { StartTime = 1200, CustomPreempt = 400, Position = Vector2.Zero }
            };

            performTest(hitObjects, new List<ReplayFrame>
            {
                new OsuReplayFrame { Time = 750, Position = Vector2.Zero, Actions = { OsuAction.LeftButton } },
                new OsuReplayFrame { Time = 760, Position = Vector2.Zero },
                new OsuReplayFrame { Time = 850, Position = Vector2.Zero, Actions = { OsuAction.RightButton } },
                new OsuReplayFrame { Time = 860, Position = Vector2.Zero }
            });

            AddStep("seek to 800", () => currentPlayer.Seek(800));
            AddUntilStep("Wait for completion again", () => currentPlayer.ScoreProcessor.HasCompleted.Value);
        }

        private class CustomPreemptHitCircle : TestHitCircle
        {
            public double CustomPreempt { get; set; }
