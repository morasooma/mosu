...
namespace osu.Game.Rulesets.Osu.Tests
{
    ...
    public partial class TestSceneOsuTouchInput : OsuManualInputManagerTestScene
    {
        ...
        [Test]
        public void TestVirtualCursorInputDelay()
        {
            Vector2 initialPosition = Vector2.Zero;
            Vector2 targetPosition = Vector2.Zero;

            AddStep("Enable virtual cursor delay", () => config.SetValue(OsuSetting.ForkVirtualCursorInputDelay, true));
            AddStep("Store initial cursor position", () => initialPosition = osuInputManager.CurrentState.Mouse.Position);
            AddStep("Move mouse", () =>
            {
                targetPosition = getSanePositionForSource(TouchSource.Touch3);
                InputManager.MoveMouseTo(targetPosition);
            });

            AddAssert("Original input updated", () => osuInputManager.OriginalUserCursorPosition, () => Is.EqualTo(targetPosition));
            AddAssert("Gameplay cursor still delayed", () => osuInputManager.CurrentState.Mouse.Position, () => Is.EqualTo(initialPosition));

            AddUntilStep("Gameplay cursor caught up", () => osuInputManager.CurrentState.Mouse.Position, () => Is.EqualTo(targetPosition));
        }
