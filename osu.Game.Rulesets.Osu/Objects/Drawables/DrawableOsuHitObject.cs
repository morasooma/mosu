...
namespace osu.Game.Rulesets.Osu.Objects.Drawables
{
    public abstract partial class DrawableOsuHitObject : DrawableHitObject<OsuHitObject>
    {
        ...
        protected void ApplyRepeatFadeIn(Drawable target, double fadeTime)
        {
            ...
            bool delayFadeIn = !SkinPerformanceMode.Enabled && slider.SliderBody?.SnakingIn.Value == true && repeatIndex == 0;
            ...
            target
                ...
                .FadeIn(SkinPerformanceMode.Enabled ? 0 : fadeTime);
        }
        ...
        protected override void OnResultApplied(JudgementResult result)
        {
            ...
            try
            {
                osu.Framework.Graphics.Drawable current = this;
                osu.Game.Rulesets.Osu.UI.DrawableOsuRuleset ruleset = null;

                while (current != null)
                {
                    if (current is osu.Game.Rulesets.Osu.UI.DrawableOsuRuleset r)
                    {
                        ruleset = r;
                        break;
                    }
                ...
                if (ruleset != null && ruleset.IsVisualOD11)
                {
                    if (result.Type == osu.Game.Rulesets.Scoring.HitResult.Great ||
                        result.Type == osu.Game.Rulesets.Scoring.HitResult.Ok ||
                        result.Type == osu.Game.Rulesets.Scoring.HitResult.Meh ||
                        result.Type == osu.Game.Rulesets.Scoring.HitResult.Miss)
                    {
                        var visualHitWindows = ruleset.FirstAvailableVisualHitWindows;
                        if (visualHitWindows != null)
                        {
                            var newResult = visualHitWindows.ResultFor(result.TimeOffset);
                            if (newResult == osu.Game.Rulesets.Scoring.HitResult.None)
                                newResult = osu.Game.Rulesets.Scoring.HitResult.Miss;
                                
                            result.VisualType = newResult;
                        }
                    }
                }
            }
