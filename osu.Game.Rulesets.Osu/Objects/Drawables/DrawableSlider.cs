...
namespace osu.Game.Rulesets.Osu.Objects.Drawables
{
    public partial class DrawableSlider : DrawableOsuHitObject
    {
        ...
        private int performanceRepeatSnakingPathVersion = int.MinValue;
        ...
        private void load()
        {
            ...
            {
                SliderInputManager,
                shakeContainer = new StableLifetimeShakeContainer
                {
                    ShakeDuration = 30,
        ...
        protected override void OnApply()
        {
            ...
            performanceRepeatSnakingPathVersion = int.MinValue;
        }
        ...
        protected override void UpdateAfterChildren()
        {
            ...
            if (sliderBody != null)
            {
                ...
                {
                    if (performanceRepeatSnakingPathVersion != pathVersion.Value)
                    {
                        ...
                        performanceRepeatSnakingPathVersion = pathVersion.Value;
                    }
                }
                else
                {
        ...
        protected override void UpdateInitialTransforms()
        {
            ...
            Body.FadeInFromZero(SkinPerformanceMode.Enabled ? 0 : HitObject.TimeFadeIn);
        }
            ...
            switch (state)
            {
                case ArmedState.Hit:
                    if (HeadCircle.IsHit && SliderBody?.SnakingOut.Value == true)
                        Body.FadeOut(SkinPerformanceMode.Enabled ? 0 : 40); // short fade to allow for any body colour to smoothly disappear.
                    break;
            }

            if (SkinPerformanceMode.Enabled)
            {
                // Keep the drawable alive for the original post-slider lifetime. Short sliders can end before the
                // head circle's miss window has elapsed, and killing the parent at EndTime leaves head/tail unjudged.
                this.FadeOut();
                LifetimeEnd = HitStateUpdateTime + fade_out_time;
            }
            else
