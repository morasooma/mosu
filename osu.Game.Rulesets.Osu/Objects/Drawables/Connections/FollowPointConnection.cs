...
namespace osu.Game.Rulesets.Osu.Objects.Drawables.Connections
{
    ...
    public partial class FollowPointConnection : PoolableDrawableWithLifetime<FollowPointLifetimeEntry>
    {
        ...
        protected override bool CheckChildrenLife()
        {
            ...
            directChildrenLifeStable = InternalChildren.Count > 0 && InternalChildren.Count == AliveInternalChildren.Count;
            return aliveChanged;
        }
        ...
        private void scheduleRefresh() => Scheduler.AddOnce(() =>
        {
            ...
            int spacing = SkinPerformanceMode.Enabled ? SPACING * 2 : SPACING;
            ...
            for (int d = (int)(spacing * 1.5); d < distance - spacing; d += spacing)
            {
                ...
                {
                    fp.Position = pointEndPosition;
                    fp.Scale = new Vector2(end.Scale);

                    using (fp.BeginAbsoluteSequence(fadeInTime))
                    {
                        fp.FadeIn();
                        fp.Delay(fadeOutTime - fadeInTime).FadeOut().Expire();
