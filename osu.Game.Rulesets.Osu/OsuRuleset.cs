...
namespace osu.Game.Rulesets.Osu
{
    public class OsuRuleset : Ruleset, ILegacyRuleset, IHasLegacyScoreV1Processor
    {
        ...
        public ILegacyScoreV1Processor CreateLegacyScoreV1Processor(IWorkingBeatmap workingBeatmap, IBeatmap playableBeatmap, IReadOnlyList<Mod> mods)
            => new OsuLegacyScoreV1Processor(workingBeatmap, mods);
        ...
        public override BeatmapDifficulty GetAdjustedDisplayDifficulty(IBeatmapInfo difficulty, IReadOnlyCollection<Mod> mods)
        {
            ...
            bool lockDifficulty = mods.Any(m => m is ModRateAdjust rateAdjust && rateAdjust.LockDifficultyAdjust.Value)
                               || mods.Any(m => m is OsuModMosuStaticBpm staticBpm && staticBpm.LockDifficultyAdjust.Value)
                               || mods.Any(m => m is OsuModMosuTargetDifficulty targetDiff && targetDiff.LockDifficultyAdjust.Value);
        ...
        public override IEnumerable<RulesetBeatmapAttribute> GetBeatmapAttributesForDisplay(IBeatmapInfo beatmapInfo, IReadOnlyCollection<Mod> mods)
        {
            ...
            bool lockDifficulty = mods.Any(m => m is ModRateAdjust rateAdjust && rateAdjust.LockDifficultyAdjust.Value)
                               ...
                               || mods.Any(m => m is OsuModMosuTargetDifficulty targetDiff && targetDiff.LockDifficultyAdjust.Value);
            double rate = ModUtils.CalculateRateWithMods(mods);
            double displayRate = lockDifficulty ? 1.0 : rate;
