...
namespace osu.Game.Rulesets.Osu.Mods
{
    public class OsuModRepel : Mod, IUpdatableByPlayfield, IApplicableToDrawableRuleset<OsuHitObject>
    {
        ...
        {
            if (!tryGetSafeDampLength(hitObject.Position, cursorPos, out double dampLength))
                return;
        ...
        private bool tryGetSafeDampLength(Vector2 position, Vector2 cursorPos, out double dampLength)
        {
            float strength = RepulsionStrength.Value;
            ...
            double distance = Vector2.Distance(position, cursorPos);
            ...
            // Keep the original formula/sign behaviour, only rejecting values that would feed invalid numbers into interpolation.
            double dampDivisor = 0.04 * strength + 0.04;

            if (!double.IsFinite(dampDivisor) || dampDivisor == 0)
            {
                dampLength = default;
                return false;
            }

            dampLength = distance / dampDivisor;
            return double.IsFinite(dampLength);
        }
    }
}
