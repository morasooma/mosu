...
namespace osu.Game.Rulesets.Osu.UI.Cursor
{
    public partial class OsuCursorContainer : GameplayCursorContainer, IKeyBindingHandler<OsuAction>
    {
        ...
        public bool RequiresHighFrequencyRenderSimulation =>
            cursorTrail.Drawable is CursorTrail trail && trail.RequiresHighFrequencyRenderSimulation;

        public string ActiveTrailDrawableType =>
            cursorTrail.Drawable?.GetType().FullName ?? "none";

        public bool IsLegacyDisjointTrail =>
            cursorTrail.Drawable is LegacyCursorTrail legacyTrail && legacyTrail.DisjointTrail;

        public string ActiveTrailPlacementAlgorithm =>
            cursorTrail.Drawable is LegacyCursorTrail legacyTrail ? legacyTrail.PlacementAlgorithm : "default_interpolated";
        ...
        private bool suppressTrail;

        public bool SuppressTrail
        {
            get => suppressTrail;
            set
            {
                if (suppressTrail == value)
                    return;
        ...
        {
            cursorTrail.FadeTo(!SuppressTrail && showTrail.Value ? 1 : 0, 200);
        }
        ...
        public override void PrepareForDeterministicRender(IFrameBasedClock gameplayClock)
        {
            if (cursorTrail.Drawable is CursorTrail trail)
            {
                trail.UseExternalClock(gameplayClock);
                trail.Reset();
            }
        }
