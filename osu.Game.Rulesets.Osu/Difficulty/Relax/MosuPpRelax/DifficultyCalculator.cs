...
internal sealed class Skill
{
    ...
    private float currentStrainForPp = 1f;
    private float currentSectionPeakForPp = 1f;
    private float? cachedDifficultyValueForPp;
    ...
    public List<float> StrainPeaksForPp { get; } = new(128);
    public List<float> ObjectStrainsForPp { get; } = new();
    ...
    public void SaveCurrentPeak()
    {
        ...
        StrainPeaksForPp.Add(currentSectionPeakForPp);
    }
    ...
    public void StartNewSectionFrom(float time)
    {
        ...
        currentSectionPeakForPp = currentStrainForPp * StrainDecay(time - previousTime.Value);
    }

    public void Process(DifficultyObject current, float penalty = 1f)
    {
        currentStrain *= StrainDecay(current.Delta);
        float objectStrain = StrainValueOf(current, recentObjects) * SkillMultiplier();
        currentStrain += objectStrain;

        ObjectStrains.Add(currentStrain);
        currentSectionPeak = MathF.Max(currentSectionPeak, currentStrain);
        currentStrainForPp *= StrainDecay(current.Delta);
        currentStrainForPp += objectStrain * penalty;
        ObjectStrainsForPp.Add(currentStrainForPp);
        currentSectionPeakForPp = MathF.Max(currentSectionPeakForPp, currentStrainForPp);
    ...
    public float DifficultyValue() => computeDifficultyValue(StrainPeaks, ref cachedDifficultyValue);
    public float DifficultyValueForPp() => computeDifficultyValue(StrainPeaksForPp, ref cachedDifficultyValueForPp);

    private float computeDifficultyValue(List<float> peaks, ref float? cached)
    {
        if (cached.HasValue)
            return cached.Value;
        if (peaks.Count == 0)
            return 0f;
        var copy = new List<float>(peaks);
        int sectionCount = copy.Count;
        int earlySectionCount = (int)MathF.Floor(sectionCount * 0.85f);
        if (sectionCount > 5 && earlySectionCount > 0)
        {
            float earlyMax = 0f;
            for (int k = 0; k < earlySectionCount; k++)
                earlyMax = MathF.Max(earlyMax, copy[k]);
            float spikeThreshold = MathF.Max(earlyMax * 1.35f, 1f);
            for (int k = earlySectionCount; k < sectionCount; k++)
            {
                float peak = copy[k];
                if (peak > spikeThreshold)
                    copy[k] = spikeThreshold + (peak - spikeThreshold) * 0.62f;
            }
        }
        copy.Sort((a, b) => b.CompareTo(a));
        float difficulty = 0f;
        float weight = 1f;
        foreach (float strain in copy)
        {
    ...
    public float CountDifficultStrains() => countDifficultStrains(ObjectStrains, cachedDifficultyValue ?? DifficultyValue());
    public float CountDifficultStrainsForPp() => countDifficultStrains(ObjectStrainsForPp, cachedDifficultyValueForPp ?? DifficultyValueForPp());

    private static float countDifficultStrains(List<float> strains, float difficultyValue)
    {
...
internal static class RxDifficultyCalculator
{
        ...
        bool applyHighCsStreamCompression)
    {
       ...
       readingObjects.Add(new ReadingObject(previous.Time, 0f, 0f, 0f, null, null));
        ...
        int pointSpamCircleCount = 0;
        bool inPointSpamRun = false;
       ...
       PreparedOsuObject current = prepared[1];
        ...
        bool isSecondSpam = IsStrictPointSpamTransition(second, previous, current);
        if (isSecondSpam)
            pointSpamCircleCount++;
        float secondPenalty = isSecondSpam ? 0f : 1f;
        aim.Process(second, secondPenalty);
        speed.Process(second, secondPenalty);
        ...
        for (int index = 2; index < prepared.Count; index++)
        {
            ...
            bool isSpam = IsStrictPointSpamTransition(h, previous, current);
            if (isSpam)
            {
                pointSpamCircleCount += inPointSpamRun ? 1 : 2;
                inPointSpamRun = true;
            }
            else
            {
                inPointSpamRun = false;
            }
            float penalty = isSpam ? 0f : 1f;
            aim.Process(h, penalty);
            speed.Process(h, penalty);
        ...
        float baseAimStrainForPp = MathF.Sqrt(aim.DifficultyValueForPp()) * DIFFICULTY_MULTIPLIER;
        ...
        float sustainedWideJumpWeight = jumpPresence
                                        * jumpCompetition
                                        * FlowSmoothStep(aim.StrainPeaks.Count, 100f, 240f);
        ...
        float aimStrainForPp = baseAimStrainForPp * (1f + FLOW_MAX_AIM_STRAIN_BONUS * effectiveMapFlowWeight);
        ...
        float speedStrainForPp = MathF.Sqrt(speed.DifficultyValueForPp()) * DIFFICULTY_MULTIPLIER;
        float aimDifficultStrainCount = aim.CountDifficultStrains();
        float aimDifficultStrainCountForPp = aim.CountDifficultStrainsForPp();
        float speedDifficultStrainCount = speed.CountDifficultStrains();
        float speedDifficultStrainCountForPp = speed.CountDifficultStrainsForPp();
        ...
        if (applyHighCsStreamCompression)
        {
        ...
        {
            sustainedWideJumpWeight = (float)canonical.SustainedWideJumpWeight;
            ...
            if (streamWeight > 0f)
            {
                ...
                aimStrainForPp = CompressPositiveStrainDelta((float)canonical.AimStrainForPp, aimStrainForPp, deltaRetention);
        ...
        attributes.AimDifficultStrainCountForPp = aimDifficultStrainCountForPp;
        attributes.SpeedDifficultStrainCount = speedDifficultStrainCount;
        attributes.SpeedDifficultStrainCountForPp = speedDifficultStrainCountForPp;
        ...
        attributes.AimPeakRatio = PeakToTypicalRatio(aim.StrainPeaks);
        attributes.SpeedPeakRatio = PeakToTypicalRatio(speed.StrainPeaks);
        attributes.CustomFlowAimBonusRatio = customFlowAimBonusRatio;
        attributes.WideFlowPatternWeight = wideFlowPatternWeight;
        attributes.SustainedWideJumpWeight = sustainedWideJumpWeight;
    ...
    internal static bool IsStrictPointSpamTransition(DifficultyObject h, PreparedOsuObject previous, PreparedOsuObject current)
    {
        bool isCircleToCircle = !previous.IsSpinner && previous.TravelDistance.GetValueOrDefault(0f) == 0f &&
                                !current.IsSpinner && current.TravelDistance.GetValueOrDefault(0f) == 0f;
        return isCircleToCircle && h.Delta <= 100f && h.JumpDistance < 3f;
    }
    ...
    /// <summary>
    /// MosuPp "Spike Nerf (RX)": how far the hardest part of the map sticks out of the rest.
    /// Average of the 10 hardest sections divided by the 75th percentile of played sections
    /// (sections below 10% of the maximum, i.e. breaks and silence, are ignored).
    /// </summary>
    private static float PeakToTypicalRatio(IReadOnlyList<float> peaks)
    {
        if (peaks.Count == 0)
            return 1f;
        ...
        var played = new List<float>(peaks.Count);
        foreach (float peak in peaks)
        {
            if (peak > max * 0.10f)
                played.Add(peak);
        ...
        // Too short to say anything about the shape of the map.
        if (played.Count < 20)
            return 1f;

        played.Sort();

        int topCount = Math.Min(10, played.Count);
        float top = 0f;
        for (int i = played.Count - topCount; i < played.Count; i++)
            top += played[i];
        top /= topCount;

        float typical = MathF.Max(played[(int)MathF.Round((played.Count - 1) * 0.75f)], 0.0001f);
        return MathF.Max(top / typical, 1f);
    }
