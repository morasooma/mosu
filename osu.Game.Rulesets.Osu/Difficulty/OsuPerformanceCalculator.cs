...
namespace osu.Game.Rulesets.Osu.Difficulty
{
    public class OsuPerformanceCalculator : PerformanceCalculator
    {
        ...
        /// <summary>
        /// Restores the historical Relax aim scale while keeping the Relax pattern
        /// anti-abuse reductions applied by <see cref="Skills.Aim"/>.
        /// </summary>
        internal const double RELAX_AIM_MULTIPLIER = 1.40;
        ...
        protected override PerformanceAttributes CreatePerformanceAttributes(ScoreInfo score, DifficultyAttributes attributes)
        {
            ...
            // Mosu rate mods can intentionally keep AR/OD at the original clock-rate values.
            // Keep this fork behaviour while using the current upstream PP algorithm.
            bool lockDifficulty = score.Mods.Any(m => m is ModRateAdjust rateAdjust && rateAdjust.LockDifficultyAdjust.Value)
                                  || score.Mods.Any(m => m is OsuModMosuStaticBpm staticBpm && staticBpm.LockDifficultyAdjust.Value)
                                  || score.Mods.Any(m => m is OsuModMosuTargetDifficulty targetDifficulty && targetDifficulty.LockDifficultyAdjust.Value);

            greatHitWindow = lockDifficulty ? hitWindows.WindowFor(HitResult.Great) : hitWindows.WindowFor(HitResult.Great) / clockRate;
            okHitWindow = lockDifficulty ? hitWindows.WindowFor(HitResult.Ok) : hitWindows.WindowFor(HitResult.Ok) / clockRate;
            mehHitWindow = lockDifficulty ? hitWindows.WindowFor(HitResult.Meh) : hitWindows.WindowFor(HitResult.Meh) / clockRate;

            approachRate = lockDifficulty ? difficulty.ApproachRate : calculateRateAdjustedApproachRate(difficulty.ApproachRate, clockRate);
        ...
        private double computeAimValue(ScoreInfo score, OsuDifficultyAttributes attributes)
        {
            ...
            // Preserve the fork's anti-abuse PP penalty on top of the current upstream formula.
            if (score.BeatmapInfo?.Status.BypassesMappingAntiAbuse() != true &&
                score.Mods.OfType<OsuModMosuAimAssist>().FirstOrDefault() is OsuModMosuAimAssist aimAssist)
                aimValue *= DiffUtils.Pow(1.0 - aimAssist.Strength.Value, 1.25);
        ...
        {
            if (score.Mods.Any(h => h is OsuModRelax or OsuModMosuRelax) || speedDeviation == null)
                return 0.0;
