...
namespace osu.Game.Rulesets.Osu.Skinning.Argon
{
    public partial class ArgonSpinnerProgressArc : CompositeDrawable
    {
        ...
        {
            // The additive glow pass is purely decorative and expensive: it draws this disc-sized arc quad
            // inflated by the sigma-50 blur kernel (~108 units per side) through the SpinnerGlow shader —
            // ~360k spinner-local px per arc, twice per spinner, while the spinner is actively spun. The
            // arc fill underneath (the actual progress display) is untouched by the gate.
            private readonly bool dropGlow = SkinPerformanceMode.ShouldSimplifyEffects && MosuOptimisationToggles.PerfSpinnerLite;
            ...
            // In spinner-lite mode the plain CircularProgress draw node is used, skipping the glow pass
            // (and its shader bind) entirely.
            protected override DrawNode CreateDrawNode() => dropGlow ? base.CreateDrawNode() : new ProgressFillDrawNode(this);
