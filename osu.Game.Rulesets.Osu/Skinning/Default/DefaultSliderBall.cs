...
{
    public partial class DefaultSliderBall : DirectChildrenLifeOptimisingCompositeDrawable
    {
        ...
        private void load(ISkinSource skin, TextureStore textures)
        {
            ...
            {
                // The regular fallback uses a masked circular framebuffer with a border.
                // The bundled disc texture already has the required circular silhouette,
                // allowing the moving ball to be represented by one additive quad.
                InternalChild = box = new Sprite
                {
                    RelativeSizeAxes = Axes.Both,
                ...
                {
                    var performanceSlider = (DrawableSlider)parentObject;
                    performanceSlider.Tracking.BindValueChanged(trackingChanged, true);
                }
        ...
        private void trackingChanged(ValueChangedEvent<bool> tracking) =>
            box.FadeTo(tracking.NewValue ? 0.3f : 0.05f, SkinPerformanceMode.Enabled ? 0 : 200, Easing.OutQuint);
