...
{
    public partial class DefaultSpinnerDisc : DirectChildrenLifeOptimisingCompositeDrawable
    {
        ...
        protected override void Update()
        {
            ...
            {
                // Keep all gameplay information (progress, tracking and rotation), but avoid
                // continuously damped/interpolated cosmetic motion in performance mode.
                fill.Alpha = drawableSpinner.RotationTracker.Tracking ? tracking_alpha : idle_alpha;

                const float performance_initial_fill_scale = 0.2f;
                float performanceTargetScale = performance_initial_fill_scale + (1 - performance_initial_fill_scale) * drawableSpinner.Progress;
                fill.Scale = new Vector2(performanceTargetScale);
