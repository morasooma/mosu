    ...
    public class CatchRuleset : Ruleset, ILegacyRuleset
    {
        ...
        public override IEnumerable<Mod> GetModsFor(ModType type)
        {
            switch (type)
            {
                case ModType.Mosu:
                    return new Mod[]
                    {
                        new CatchModMosuStaticBpm(),
                        new CatchModMosuTargetDifficulty(),
                        new CatchModAudioEffects(),
                    };
        ...
        public override BeatmapDifficulty GetAdjustedDisplayDifficulty(IBeatmapInfo beatmapInfo, IReadOnlyCollection<Mod> mods)
        {
            ...
            bool lockDifficulty = mods.Any(m => m is ModRateAdjust rateAdjust && rateAdjust.LockDifficultyAdjust.Value)
                               || mods.Any(m => m is ModMosuStaticBpm staticBpm && staticBpm.LockDifficultyAdjust.Value)
                               || mods.Any(m => m is ModMosuTargetDifficulty targetDiff && targetDiff.LockDifficultyAdjust.Value);
