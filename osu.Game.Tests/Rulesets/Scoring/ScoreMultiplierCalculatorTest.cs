...
namespace osu.Game.Tests.Rulesets.Scoring
{
    public class ScoreMultiplierCalculatorTest
    {
        ...
        [Test]
        public void TestDuplicateModsAreOnlyAppliedOnce()
        {
            var calculator = new TestScoreMultiplierCalculator(new ScoreMultiplierContext(new BeatmapDifficulty()));

            double multiplier = calculator.CalculateFor([new OsuModEasy(), new OsuModEasy()]);

            Assert.That(multiplier, Is.EqualTo(0.15));
        }

        [Test]
        public void TestDuplicateUnregisteredModsDoNotThrow()
        {
            var calculator = new TestScoreMultiplierCalculator(new ScoreMultiplierContext(new BeatmapDifficulty()));

            Assert.That(() => calculator.CalculateFor([new OsuModHidden(), new OsuModHidden()]), Throws.Nothing);
        }
