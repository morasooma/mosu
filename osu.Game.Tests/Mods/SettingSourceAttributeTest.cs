...
namespace osu.Game.Tests.Mods
{
    ...
    public partial class SettingSourceAttributeTest
    {
        ...
        [Test]
        public void TestNumericInputAddedWhenRequested()
        {
            var objectWithSettings = new ClassWithSettings();
            var settings = objectWithSettings.CreateSettingsControls(true).ToArray();

            Assert.That(settings, Has.Length.EqualTo(7));
            Assert.That(settings.OfType<SettingsNumericTextBox<double>>(), Has.Count.EqualTo(1));
            Assert.That(settings.OfType<SettingsNumericTextBox<int>>(), Has.Count.EqualTo(1));
            Assert.That(settings.OfType<SettingsNumericTextBox<float>>(), Has.Count.EqualTo(1));
        }
