...
namespace osu.Game.Tests.Online
{
    ...
    public class TestAPIModJsonSerialization
    {
        ...
        [Test]
        public void TestConvertedModHasCorrectExtraApiSetting()
        {
            var apiMod = new APIMod(new TestModWithApiExtension
            {
                TestSetting = { Value = 2 },
                DerivedSetting = { Value = 1.2345 },
            });

            var deserialized = JsonConvert.DeserializeObject<APIMod>(JsonConvert.SerializeObject(apiMod));
            var converted = (TestModWithApiExtension)deserialized?.ToMod(new TestRuleset());

            Assert.Multiple(() =>
            {
                Assert.That(deserialized?.Settings, Contains.Key("derived_setting").With.ContainValue(1.2345));
                Assert.That(converted?.DerivedSetting.Value, Is.EqualTo(1.2345).Within(0.000001));
            });
        }
        ...
        [Test]
        public void TestDeserialiseSoloScoreWithTargetDifficultySpeedChange()
        {
            var score = SoloScoreInfo.ForSubmission(new ScoreInfo
            {
                Mods = new Mod[]
                {
                    new OsuModMosuTargetDifficulty
                    {
                        TargetDifficulty = { Value = 6.5 },
                        SpeedChange = { Value = 0.61 },
                    }
                },
                User = new APIUser(),
                Ruleset = new OsuRuleset().RulesetInfo,
            ...
            var serialised = JsonConvert.SerializeObject(score);
            var deserialised = JsonConvert.DeserializeObject<SoloScoreInfo>(serialised);

            Assert.Multiple(() =>
            {
                Assert.That(serialised, Does.Contain(@"""speed_change"":0.61"));
                Assert.That(deserialised?.Mods[0]?.Acronym, Is.EqualTo("TS"));
                Assert.That((double?)deserialised?.Mods[0]?.Settings["speed_change"], Is.EqualTo(0.61).Within(0.000001));
            });
        }
        ...
        private class TestModWithApiExtension : TestMod, IProvideAPIModSettings
        {
            public override string Acronym => "TMX";

            public BindableDouble DerivedSetting { get; } = new BindableDouble(1)
            {
                Precision = 0.0001,
            };

            public void AddAPIModSettings(Dictionary<string, object> settings) => settings["derived_setting"] = DerivedSetting.Value;

            public bool TryApplyAPIModSetting(string settingKey, object settingValue)
                => settingKey == "derived_setting" && DerivedSetting.TrySetExactNumericValue(settingValue);
        }
