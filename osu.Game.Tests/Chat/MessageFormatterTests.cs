...
namespace osu.Game.Tests.Chat
{
    ...
    public class MessageFormatterTests
    {
        ...
        [Test]
        public void TestDownloadBeatmapSetProtocolLink()
        {
            Message result = MessageFormatter.FormatMessage(new Message { Content = "osu://dl/123" });

            ClassicAssert.AreEqual(1, result.Links.Count);
            ClassicAssert.AreEqual(LinkAction.DownloadBeatmapSet, result.Links[0].Action);
            ClassicAssert.AreEqual("123", result.Links[0].Argument);
        }
