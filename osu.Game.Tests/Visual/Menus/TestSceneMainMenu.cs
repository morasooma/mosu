...
namespace osu.Game.Tests.Visual.Menus
{
    public partial class TestSceneMainMenu : OsuGameTestScene
    {
        ...
        [Test]
        public void TestInterfaceShearSettingUpdatesLoadedMainMenu()
        {
            AddStep("enable interface shear", () => Game.LocalConfig.SetValue(OsuSetting.ForkDisableInterfaceShear, false));
            AddStep("enter menu", () => InputManager.Key(Key.P));
            AddStep("enter play submenu", () => InputManager.Key(Key.P));
            AddUntilStep("play buttons visible", () => Game.ChildrenOfType<MainMenuButton>().Count(b => b.IsPresent), () => Is.GreaterThanOrEqualTo(3));
            AddAssert("buttons are sheared", () => Game.ChildrenOfType<MainMenuButton>().Where(b => b.IsPresent).All(b => b.BackgroundShear != Vector2.Zero));

            AddStep("disable interface shear in config", () => Game.LocalConfig.SetValue(OsuSetting.ForkDisableInterfaceShear, true));
            AddUntilStep("loaded buttons become flat", () => Game.ChildrenOfType<MainMenuButton>().Where(b => b.IsPresent).All(b => b.BackgroundShear == Vector2.Zero));
            AddUntilStep("button spacing becomes flat", () => Game.ChildrenOfType<ButtonArea>().Single().Flow.Spacing, () => Is.EqualTo(Vector2.Zero));

            AddStep("restore interface shear", () => Game.LocalConfig.SetValue(OsuSetting.ForkDisableInterfaceShear, false));
        }
