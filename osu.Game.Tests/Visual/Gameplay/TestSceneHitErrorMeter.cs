...
namespace osu.Game.Tests.Visual.Gameplay
{
    public partial class TestSceneHitErrorMeter : OsuTestScene
    {
        ...
        [Cached(typeof(IPositionalMissProvider))]
        private TestPositionalMissProvider positionalMissProvider = new TestPositionalMissProvider();
        ...
        public void SetUp()
        {
            ...
            AddStep("disable positional misses", () => config.SetValue(OsuSetting.ForkHitErrorMeterShowPositionalMisses, false));
        }

        [Test]
        public void TestPositionalMisses()
        {
            AddStep("create display", () => recreateDisplay(new OsuHitWindows(), 5));

            AddStep("positional miss while disabled", () => positionalMissProvider.Trigger(50));
            AddAssert("no positional markers", () => !this.ChildrenOfType<BarHitErrorMeter.JudgementLine>().Any());

            AddStep("enable positional misses", () => config.SetValue(OsuSetting.ForkHitErrorMeterShowPositionalMisses, true));
            AddStep("positional miss", () => positionalMissProvider.Trigger(-50));
            AddAssert("one marker per bar", () => this.ChildrenOfType<BarHitErrorMeter>().All(
                meter => meter.ChildrenOfType<BarHitErrorMeter.JudgementLine>().Count() == 1));
            AddAssert("markers use positional miss style", () => this.ChildrenOfType<BarHitErrorMeter.JudgementLine>().All(
                marker => marker.IsPositionalMiss && marker.Scale.X == 1 && marker.Scale.Y == 1));

            AddStep("regular hit", () => newJudgement(25));
            AddAssert("regular markers keep normal style", () => this.ChildrenOfType<BarHitErrorMeter>().All(meter =>
                meter.ChildrenOfType<BarHitErrorMeter.JudgementLine>().Any(marker => !marker.IsPositionalMiss && marker.Scale.X == 1)));
        }
        ...
        private class TestPositionalMissProvider : IPositionalMissProvider
        {
            public event Action<double> NewPositionalMiss;

            public void Trigger(double timeOffset) => NewPositionalMiss?.Invoke(timeOffset);
        }
    }
}
