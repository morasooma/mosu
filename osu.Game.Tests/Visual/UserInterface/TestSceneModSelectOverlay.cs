...
namespace osu.Game.Tests.Visual.UserInterface
{
    ...
    public partial class TestSceneModSelectOverlay : ScreenTestScene
    {
        ...
        public override void SetUpSteps()
        {
            ...
            AddStep("reset classic mod default", () => configManager.SetValue(OsuSetting.ForkClassicModDefault, false));
            AddStep("reset fork mod numeric input", () => configManager.SetValue(OsuSetting.ForkEnableModNumericInput, false));
            AddStep("show mods in presets", () => configManager.SetValue(OsuSetting.ForkShowModsInPresetList, true));
        ...
        [Test]
        public void TestPresetModAcronymsCanBeShownInDescription()
        {
            createScreen();

            AddAssert("one mod icon created", () => getPresetPanel("AR0").ChildrenOfType<ModIcon>().Count(), () => Is.EqualTo(1));
            AddAssert("mod icons shown", () => getIconFlow("AR0").IsPresent);
            AddStep("disable mod acronyms", () => configManager.SetValue(OsuSetting.ForkShowModsInPresetList, false));
            AddAssert("mod icons hidden", () => !getIconFlow("AR0").IsPresent);
            AddAssert("original description retained", () => getPresetPanel("AR0").Description.ToString(), () => Is.EqualTo("Too... many... circles..."));

            ModPresetPanel getPresetPanel(string title) => modSelectOverlay.ChildrenOfType<ModPresetPanel>().Single(panel => panel.Title.ToString() == title);
            FillFlowContainer getIconFlow(string title) => getPresetPanel(title).ChildrenOfType<FillFlowContainer>().Single(flow => flow.Children.OfType<ModIcon>().Any());
        }
        ...
        [Test]
        public void TestExactNumericInputShownWhenEnabled()
        {
            createScreen();

            AddStep("enable exact numeric input", () => configManager.SetValue(OsuSetting.ForkEnableModNumericInput, true));
            AddStep("select DT", () => SelectedMods.Value = new[] { new OsuModDoubleTime() });
            AddAssert("exact numeric input shown", () => modSelectOverlay.ChildrenOfType<SettingsNumericTextBox<double>>().Any());
            AddStep("select difficulty adjust", () => SelectedMods.Value = new[] { new OsuModDifficultyAdjust() });
            AddAssert("difficulty exact input shown", () => modSelectOverlay.ChildrenOfType<SettingsDifficultyTextBox>().Any());
        }
        ...
        [Test]
        public void TestDeselectAllViaKeyPreservesDefaultClassicMod()
        {
            AddStep("enable classic mod default", () => configManager.SetValue(OsuSetting.ForkClassicModDefault, true));
            createScreen();
            changeRuleset(0);

            AddStep("kill search bar focus", () => modSelectOverlay.SearchTextBox.KillFocus());
            AddStep("select DT + HD + CL", () => SelectedMods.Value = new Mod[] { new OsuModDoubleTime(), new OsuModHidden(), new OsuModClassic() });

            AddStep("press backspace", () => InputManager.Key(Key.BackSpace));
            AddUntilStep("only classic mod remains", () => SelectedMods.Value, () => Is.EqualTo(new Mod[] { new OsuModClassic() }));

            AddStep("disable classic manually", () => getPanelForMod(typeof(OsuModClassic)).TriggerClick());
            AddUntilStep("classic mod disabled", () => !SelectedMods.Value.Any());
            AddStep("enable classic manually", () => getPanelForMod(typeof(OsuModClassic)).TriggerClick());
            AddUntilStep("classic mod enabled", () => SelectedMods.Value, () => Is.EqualTo(new Mod[] { new OsuModClassic() }));
        }
