...
namespace osu.Game.Tests.Visual.UserInterface
{
    ...
    public partial class TestSceneButtonSystem : OsuManualInputManagerTestScene
    {
        ...
        [Test]
        public void TestSpacingUpdatesWithoutReloadingButtonSystem()
        {
            AddStep("enable interface shear", () => OsuGame.DisableShear.Value = false);
            AddAssert("buttons overlap wedges", () => buttons.ChildrenOfType<ButtonArea>().Single().Flow.Spacing, () => Is.EqualTo(new Vector2(-ButtonSystem.WEDGE_WIDTH, 0)));

            AddStep("disable interface shear", () => OsuGame.DisableShear.Value = true);
            AddAssert("flat buttons do not overlap", () => buttons.ChildrenOfType<ButtonArea>().Single().Flow.Spacing, () => Is.EqualTo(Vector2.Zero));

            AddStep("re-enable interface shear", () => OsuGame.DisableShear.Value = false);
            AddAssert("wedge overlap is restored", () => buttons.ChildrenOfType<ButtonArea>().Single().Flow.Spacing, () => Is.EqualTo(new Vector2(-ButtonSystem.WEDGE_WIDTH, 0)));
        }
