...
namespace osu.Game.Tests.Visual.UserInterface
{
    ...
    public partial class TestSceneMainMenuButton : OsuTestScene
    {
        ...
        [Test]
        public void TestShearUpdatesWithoutReloadingButton()
        {
            MainMenuButton button = null!;

            AddStep("enable interface shear", () => OsuGame.DisableShear.Value = false);
            AddStep("add button", () => Child = button = new MainMenuButton(
                ButtonSystemStrings.Solo, @"button-default-select", OsuIcon.Player, new Color4(102, 68, 204, 255))
            {
                Anchor = Anchor.Centre,
                Origin = Anchor.Centre,
                ButtonSystemState = ButtonSystemState.TopLevel,
            });
            AddAssert("button is sheared", () => button.BackgroundShear, () => Is.EqualTo(new Vector2(0.2f, 0)));

            AddStep("disable interface shear", () => OsuGame.DisableShear.Value = true);
            AddAssert("same button is flat", () => button.BackgroundShear, () => Is.EqualTo(Vector2.Zero));

            AddStep("re-enable interface shear", () => OsuGame.DisableShear.Value = false);
            AddAssert("same button is sheared again", () => button.BackgroundShear, () => Is.EqualTo(new Vector2(0.2f, 0)));
        }
