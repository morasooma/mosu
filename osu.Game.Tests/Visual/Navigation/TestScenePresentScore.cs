...
namespace osu.Game.Tests.Visual.Navigation
{
    public partial class TestScenePresentScore : OsuGameTestScene
    {
        ...
        [Test]
        public void TestReplayFromSongSelectWithRelaxRulesetReturnsToSongSelect()
        {
            AddStep("enter song select", () => Game.ChildrenOfType<ButtonSystem>().Single().OnSolo?.Invoke());
            AddUntilStep("song select is current", () => Game.ScreenStack.CurrentScreen is SoloSongSelect songSelect && songSelect.CarouselItemsPresented);

            var imported = importScore(1, mods: [new OsuModRelax()]);

            AddStep("select relax ruleset", () => Game.Ruleset.Value = new OsuRuleset().RulesetInfo.CreateSpecialRuleset(
                RulesetInfo.OSU_RELAX_MODE_SHORTNAME, RulesetInfo.OSU_RELAX_ONLINE_ID));
            AddUntilStep("relax ruleset selected", () => Game.Ruleset.Value.ShortName, () => Is.EqualTo(RulesetInfo.OSU_RELAX_MODE_SHORTNAME));

            AddStep("present relax replay", () => Game.PresentScore(imported(), ScorePresentType.Gameplay));
            AddUntilStep("wait for replay loader", () => Game.ScreenStack.CurrentScreen is ReplayPlayerLoader);
            AddAssert("correct replay selected", () => ((ReplayPlayerLoader)Game.ScreenStack.CurrentScreen).Score.Equals(imported()));

            AddStep("exit replay loader", () => Game.ScreenStack.CurrentScreen.Exit());
            AddUntilStep("returned to song select", () => Game.ScreenStack.CurrentScreen is SoloSongSelect);
            AddAssert("relax ruleset restored", () => Game.Ruleset.Value.ShortName, () => Is.EqualTo(RulesetInfo.OSU_RELAX_MODE_SHORTNAME));
        }
        ...
        private Func<ScoreInfo> importScore(int i, RulesetInfo? ruleset = null, string? hash = null, Mod[]? mods = null)
        {
