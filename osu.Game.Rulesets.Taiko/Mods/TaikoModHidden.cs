    ...
    public class TaikoModHidden : ModHidden, IApplicableToDrawableRuleset<TaikoHitObject>
    {
        ...
        protected override void ApplyNormalVisibilityState(DrawableHitObject hitObject, ArmedState state)
        {
            switch (hitObject)
            {
                ...
                case DrawableHit:
                    // Constant Speed ignores SV changes, so applying the map's SV multiplier here
                    // would make the hidden area differ from the one shown with normal scrolling.
                    double scrollMultiplier = drawableRuleset.VisualisationMethod == ScrollVisualisationMethod.Constant
                        ? 1
                        : drawableRuleset.ControlPointAt(hitObject.HitObject.StartTime).Multiplier;

                    double preempt = drawableRuleset.TimeRange.Value / scrollMultiplier;
